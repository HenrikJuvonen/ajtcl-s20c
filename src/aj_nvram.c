 ...
 *
 * @return Pointer pointing to an entry in the NVRAM if an entry with the
...
OPEN_ERR_EXIT:
  ...
  AJ_ErrPrintf(
      ("AJ_NVRAM_Open(): failure: status=%s\n", AJ_StatusText(status)));
  return NULL;
}
...
size_t AJ_NVRAM_Write(const void *ptr, uint16_t size, AJ_NV_DATASET *handle) {
  ...
  AJ_InfoPrintf(
      ("AJ_NVRAM_Write(ptr=0x%p, size=%d., handle=0x%p)\n", ptr, size, handle));
  ...
  if (bytesWrite > 0 && ((handle->curPos & 0x3) != 0)) {
    ...
    memcpy(tmpBuf, handle->inode + sizeof(NV_EntryHeader) + alignedPos,
           handle->curPos & 0x3);
    if (patchBytes > bytesWrite) {
      patchBytes = bytesWrite;
    }
    memcpy(tmpBuf + (handle->curPos & 0x3), buf, patchBytes);
    _AJ_NV_Write(handle->inode + sizeof(NV_EntryHeader) + alignedPos, tmpBuf,
                 4);
    buf += patchBytes;
  ...
  if (bytesWrite > 0) {
    _AJ_NV_Write(handle->inode + sizeof(NV_EntryHeader) + handle->curPos, buf,
                 bytesWrite);
...
size_t AJ_NVRAM_Read(void *ptr, uint16_t size, AJ_NV_DATASET *handle) {
  ...
  AJ_InfoPrintf(
      ("AJ_NVRAM_Read(ptr=0x%p, size=%d., handle=0x%p)\n", ptr, size, handle));
  ...
  if (bytesRead > 0) {
    _AJ_NV_Read(handle->inode + sizeof(NV_EntryHeader) + handle->curPos, ptr,
                bytesRead);
...
void AJ_NVRAM_Clear() { _AJ_NVRAM_Clear(); }
