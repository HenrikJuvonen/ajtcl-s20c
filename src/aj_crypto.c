 ...
 */
typedef struct _AES_Block { uint8_t data[AJ_BLOCKSZ]; } AES_Block;
 ...
 */
static void CBC_MAC(const uint8_t *key, const uint8_t *in, uint32_t len,
                    CCM_Context *context) {
  while (len >= AJ_BLOCKSZ) {
    AJ_AES_CBC_128_ENCRYPT(key, in, context->T.data, AJ_BLOCKSZ,
                           context->ivec0.data);
  ...
  if (len) {
    ZERO(context->x1.A);
    memcpy(context->x1.A.data, in, len);
    AJ_AES_CBC_128_ENCRYPT(key, context->x1.A.data, context->T.data, AJ_BLOCKSZ,
                           context->ivec0.data);
    Trace("After AES", context->T.data, AJ_BLOCKSZ);
  }
}
 ...
 */
static void Compute_CCM_AuthTag(const uint8_t *key, CCM_Context *context,
                                const uint8_t *msg, uint32_t mLen,
                                uint32_t hdrLen) {
  /*
   * Initialize CBC-MAC with B_0 initialization vector is 0.
   */
  Trace("CBC IV in", context->x1.B_0.data, AJ_BLOCKSZ);
  AJ_AES_CBC_128_ENCRYPT(key, context->x1.B_0.data, context->T.data, AJ_BLOCKSZ,
                         context->ivec0.data);
  ...
  if (hdrLen) {
    uint32_t firstFew;
    /*
     * This encodes the header data length and the first few bytes of the header
     * data
     */
    ZERO(context->x1.A);
    context->x1.A.data[0] = (uint8_t)(hdrLen >> 8);
    context->x1.A.data[1] = (uint8_t)(hdrLen >> 0);
    firstFew = min(hdrLen, 14);
    memcpy(&context->x1.A.data[2], msg, firstFew);
    /*
     ...
     */
    Trace("Before AES", context->x1.A.data, AJ_BLOCKSZ);
    AJ_AES_CBC_128_ENCRYPT(key, context->x1.A.data, context->T.data, AJ_BLOCKSZ,
                           context->ivec0.data);
    Trace("After AES", context->T.data, AJ_BLOCKSZ);
    /*
  ...
  Trace("CBC-MAC", context->T.data, AJ_BLOCKSZ);
}

static CCM_Context *InitCCMContext(const uint8_t *nonce, uint32_t nLen,
                                   uint32_t hdrLen, uint32_t msgLen,
                                   uint8_t M) {
  int i;
  int l;
  ...
  if (context) {
    ...
    /*
     * Compute the B_0 block. This encodes the flags, the nonce, and the message
     * length.
     */
    context->x1.B_0.data[0] = flags;
    memcpy(&context->x1.B_0.data[1], nonce, nLen);
    for (i = 15, l = msgLen - hdrLen; l != 0; i--) {
      context->x1.B_0.data[i] = (uint8_t)l;
      l >>= 8;
    }
  }
  return context;
 ...
 */
AJ_Status AJ_Encrypt_CCM(const uint8_t *key, uint8_t *msg, uint32_t msgLen,
                         uint32_t hdrLen, uint8_t tagLen, const uint8_t *nonce,
                         uint32_t nLen) {
   ...
   */
  AJ_AES_CTR_128(key, context->T.data, msg + msgLen, tagLen,
                 context->ivec.data);
   ...
   */
  if (msgLen != hdrLen) {
    AJ_AES_CTR_128(key, msg + hdrLen, msg + hdrLen, msgLen - hdrLen,
                   context->ivec.data);
  }
  /*
 ...
 */
AJ_Status AJ_Decrypt_CCM(const uint8_t *key, uint8_t *msg, uint32_t msgLen,
                         uint32_t hdrLen, uint8_t tagLen, const uint8_t *nonce,
                         uint32_t nLen) {
  AJ_Status status = AJ_OK;
  CCM_Context *context;
...
AJ_Status AJ_Crypto_PRF(const uint8_t **inputs, const uint8_t *lengths,
