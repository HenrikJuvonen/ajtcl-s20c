...
static void CBC_MAC(const uint8_t* key, const uint8_t* in, uint32_t len, CCM_Context* context)
{
    ...
    if (len) {
        ...
        AJ_AES_CBC_128_ENCRYPT(key, context->x1.A.data, context->T.data, AJ_BLOCKSZ, context->ivec0.data);
        Trace("After AES", context->T.data, AJ_BLOCKSZ);
    }
}
                                ...
                                uint32_t hdrLen)
{
    ...
    AJ_AES_CBC_128_ENCRYPT(key, context->x1.B_0.data, context->T.data, AJ_BLOCKSZ, context->ivec0.data);
