...
AJ_Status AJ_SerialTX_Init() {
  ...
  for (i = 0; i < AJ_SerialLinkParams.maxWindowSize; i++) {
    ...
    bufferTxFreeList->buffer = AJ_Malloc(
        SLIPPED_LEN(AJ_SerialLinkParams.packetSize)); // TODO: calculate slipped
                                                      // length based on packet
                                                      // size
    bufferTxFreeList->actualLen = 0;
    bufferTxFreeList->allocatedLen =
...
/**
 * This function is called if an acknowledgement is not received within the
 * required
 * timeout period.
 */
void ResendPackets() {
...
static void QueueUnreliable(void) {
   ...
   * structure has already been queued due to a explicit ACK or due to an
   * earlier
   * link control packet.  In any case, because these are unreliable packets it
   * is OK
  ...
  if (txQueue == txUnreliable) {
    AJ_AlwaysPrintf(
        ("QueueUnreliable: type %i unreliable packet already queued! %p\n",
...
/**
 * Reliable packets are sent in order so are appended to the end of the transmit
 * queue.
 */
...
AJ_Status AJ_SerialSend(uint8_t *buffer, uint16_t bufLen) {
...
static uint16_t SlipBytes(AJ_SlippedBuffer volatile *slip, uint8_t *data,
                          uint16_t len) {
  uint16_t i;
  uint8_t b;
...
void ConvertPacketToBytes(AJ_SlippedBuffer volatile *slip,
...
void AJ_SerialTx_ReceivedSeq(uint8_t seq) {
  ...
  /*
   * If we have hit our pending ACK limit send a explicit ACK packet
   * immediately.
   */
...
void AJ_TransmitCallback(uint8_t *buffer, uint16_t bytesWritten) {
  dataSent = 1;
  AJ_ASSERT((buffer == pendingSendBuffer->buffer) &&
            (bytesWritten == pendingSendBuffer->actualLen));
