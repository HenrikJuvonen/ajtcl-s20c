...
#define REQUIRED_AUTH_VERSION                                                  \
  (((uint32_t)MAX_AUTH_VERSION << 16) | MIN_KEYGEN_VERSION)
...
typedef enum { SASL_HANDSHAKE, ALLJOYN_HANDSHAKE } HandshakeType;
...
typedef struct _AuthContext {
  ...
  const AJ_GUID
      *peerGuid;        /* GUID pointer for the currently authenticating peer */
...
static AJ_Status KeyGen(const char *peerName, uint8_t role, const char *nonce1,
                        const char *nonce2, uint8_t *outBuf, uint32_t len) {
  AJ_Status status;
  const uint8_t *data[4];
  uint8_t lens[4];
  const AJ_GUID *peerGuid = AJ_GUID_Find(peerName);

  AJ_InfoPrintf(("KeyGen(peerName=\"%s\", role=%d., nonce1=\"%s\", "
                 "nonce2=\"%s\", outbuf=0x%p, len=%d.)\n",
  ...
  status = versionContext.PRF(data, lens, ArraySize(data), outBuf,
                              AES_KEY_LEN + AJ_VERIFIER_LEN);
  /*
...
static AJ_Status ComputeMasterSecret(uint8_t *secret, size_t secretlen) {
  ...
  status =
      versionContext.PRF(data, lens, ArraySize(data),
                         handshakeContext.mastersecret, AJ_MASTER_SECRET_LEN);

  return status;
}

static AJ_Status SaveMasterSecret(const AJ_GUID *peerGuid,
                                  uint32_t expiration) {
  AJ_Status status;
  ...
  if (peerGuid) {
    /*
     * If the authentication was succesful write the credentials for the
     * authenticated peer to
     * NVRAM otherwise delete any stale credentials that might be stored.
     */
    if (AJ_AUTH_SUCCESS == handshakeContext.state) {
      status = AJ_StorePeerSecret(peerGuid, handshakeContext.mastersecret,
                                  AJ_MASTER_SECRET_LEN, expiration);
    } else {
      AJ_WarnPrintf(
          ("SaveMasterSecret(peerGuid=0x%p, expiration=%d): Invalid state\n",
           peerGuid, expiration));
...
static AJ_Status HandshakeValid(const AJ_GUID *peerGuid) {
  ...
  if (!authContext.peerGuid) {
    AJ_InfoPrintf(
        ("AJ_HandshakeValid(peerGuid=0x%p): Invalid peer guid\n", peerGuid));
  ...
  if (AJ_OK != HandshakeTimeout()) {
    AJ_InfoPrintf(
        ("AJ_HandshakeValid(peerGuid=0x%p): Handshake timed out\n", peerGuid));
    return AJ_ERR_TIMEOUT;
  }
  /*
...
AJ_Status AJ_PeerAuthenticate(AJ_BusAttachment *bus, const char *peerName,
   ...
   */
  AJ_MarshalMethodCall(bus, &msg, AJ_METHOD_EXCHANGE_GUIDS, peerName, 0,
                       AJ_NO_FLAGS, AJ_CALL_TIMEOUT);
...
AJ_Status AJ_PeerHandleExchangeGUIDs(AJ_Message *msg, AJ_Message *reply) {
  ...
  AJ_InfoPrintf(
      ("AJ_PeerHandleExchangeGuids(msg=0x%p, reply=0x%p)\n", msg, reply));
  ...
  if (AJ_OK != status) {
    AJ_InfoPrintf(
        ("AJ_PeerHandleExchangeGuids(msg=0x%p, reply=0x%p): Unmarshal error\n",
         msg, reply));
  ...
  if (AJ_OK != status) {
    AJ_InfoPrintf(
        ("AJ_PeerHandleExchangeGuids(msg=0x%p, reply=0x%p): Invalid GUID\n",
         msg, reply));
    HandshakeComplete(AJ_ERR_SECURITY);
    return AJ_MarshalErrorMsg(msg, reply, AJ_ErrSecurityViolation);
  }
  ...
  if (AJ_OK != status) {
    AJ_InfoPrintf(("AJ_PeerHandleExchangeGuids(msg=0x%p, reply=0x%p): Add name "
                   "mapping error\n",
                   msg, reply));
  ...
  /*
   * We are not currently negotiating versions so we tell the peer what version
   * we require.
   */
  ...
  AJ_InfoPrintf(
      ("AJ_PeerHandleExchangeGuids(msg=0x%p, reply=0x%p): Version %x\n", msg,
       reply, version));
  status = SetAuthVersion(version);
  if (AJ_OK != status) {
    AJ_InfoPrintf(
        ("AJ_PeerHandleExchangeGuids(msg=0x%p, reply=0x%p): Invalid version\n",
         msg, reply));
    HandshakeComplete(AJ_ERR_SECURITY);
    return AJ_MarshalErrorMsg(msg, reply, AJ_ErrSecurityViolation);
  }
...
AJ_Status AJ_PeerHandleExchangeGUIDsReply(AJ_Message *msg) {
  ...
  if (msg->hdr->msgType == AJ_MSG_ERROR) {
    AJ_WarnPrintf(("AJ_PeerHandleExchangeGUIDsReply(msg=0x%p): error=%s.\n",
                   msg, msg->x2.error));
    if (0 == strncmp(msg->x2.error, AJ_ErrResources, sizeof(AJ_ErrResources))) {
  ...
  if (authContext.peerGuid) {
    ...
    if (AJ_ERR_TIMEOUT != status) {
      AJ_WarnPrintf(
          ("AJ_PeerHandleExchangeGUIDsReply(msg=0x%p): Handshake in progress\n",
           msg));
      return AJ_ERR_RESOURCES;
    }
  }
  ...
  if (status != AJ_OK) {
    AJ_WarnPrintf(
        ("AJ_PeerHandleExchangeGUIDsReply(msg=0x%p): Unmarshal error\n", msg));
    HandshakeComplete(AJ_ERR_SECURITY);
    return AJ_ERR_SECURITY;
  }
  version = GetAcceptableVersion(version);
  if (0 == version) {
    AJ_WarnPrintf(
        ("AJ_PeerHandleExchangeGUIDsReply(msg=0x%p): Invalid version\n", msg));
    HandshakeComplete(AJ_ERR_SECURITY);
    return AJ_ERR_SECURITY;
  }
  status = SetAuthVersion(version);
  ...
  if (AJ_OK != status) {
    AJ_WarnPrintf(
        ("AJ_PeerHandleExchangeGUIDsReply(msg=0x%p): Invalid GUID\n", msg));
    HandshakeComplete(AJ_ERR_SECURITY);
    return AJ_ERR_SECURITY;
  }
  /*
   * Two name mappings to add, the well known name, and the unique name from the
   * message.
   */
  status =
  ...
  if (AJ_OK != status) {
    AJ_WarnPrintf(
        ("AJ_PeerHandleExchangeGUIDsReply(msg=0x%p): Add name mapping error\n",
         msg));
    HandshakeComplete(AJ_ERR_RESOURCES);
    return AJ_ERR_RESOURCES;
  }
...
static AJ_Status ExchangeSuites(AJ_Message *msg) {
  ...
  AJ_MarshalMethodCall(msg->bus, &call, AJ_METHOD_EXCHANGE_SUITES, msg->sender,
                       0, AJ_NO_FLAGS, AJ_AUTH_CALL_TIMEOUT);
  status = AJ_MarshalArgs(&call, "au", msg->bus->suites,
                          msg->bus->numsuites * sizeof(uint32_t));
  if (AJ_OK != status) {
...
AJ_Status AJ_PeerHandleExchangeSuites(AJ_Message *msg, AJ_Message *reply) {
  ...
  AJ_InfoPrintf(
      ("AJ_PeerHandleExchangeSuites(msg=0x%p, reply=0x%p)\n", msg, reply));
  ...
  if (AJ_OK != status) {
    AJ_InfoPrintf(
        ("AJ_PeerHandleExchangeSuites(msg=0x%p, reply=0x%p): Unmarshal error\n",
         msg, reply));
    HandshakeComplete(AJ_ERR_SECURITY);
    return AJ_MarshalErrorMsg(msg, reply, AJ_ErrSecurityViolation);
  }
  ...
  if (!handshakeContext.suites) {
    AJ_InfoPrintf((
        "AJ_PeerHandleExchangeSuites(msg=0x%p, reply=0x%p): No common suites\n",
        msg, reply));
    HandshakeComplete(AJ_ERR_SECURITY);
    return AJ_MarshalErrorMsg(msg, reply, AJ_ErrSecurityViolation);
  }
  ...
  if (AJ_OK != status) {
    AJ_WarnPrintf(
        ("AJ_PeerHandleExchangeSuites(msg=0x%p, reply=0x%p): Marshal error\n",
         msg, reply));
    HandshakeComplete(AJ_ERR_SECURITY);
    return AJ_MarshalErrorMsg(msg, reply, AJ_ErrSecurityViolation);
  }
...
AJ_Status AJ_PeerHandleExchangeSuitesReply(AJ_Message *msg) {
  ...
  if (AJ_OK != status) {
    AJ_InfoPrintf(
        ("AJ_PeerHandleExchangeSuitesReply(msg=0x%p): Unmarshal error\n", msg));
    HandshakeComplete(AJ_ERR_SECURITY);
    return AJ_ERR_SECURITY;
  }
  numsuites /= sizeof(uint32_t);
  ...
  if (!handshakeContext.suites) {
    AJ_InfoPrintf((
        "AJ_PeerHandleExchangeSuitesReply(msg=0x%p): No common suites\n", msg));
    HandshakeComplete(AJ_ERR_SECURITY);
    return AJ_ERR_SECURITY;
  }
...
static AJ_Status KeyExchange(AJ_Message *msg) {
  ...
  if (AJ_OK != status) {
    AJ_WarnPrintf(
        ("KeyExchange(msg=0x%p): Suite 0x%x not available\n", msg, suite));
    HandshakeComplete(AJ_ERR_SECURITY);
    return AJ_ERR_SECURITY;
  }
   ...
   */
  AJ_MarshalMethodCall(msg->bus, &call, AJ_METHOD_KEY_EXCHANGE, msg->sender, 0,
                       AJ_NO_FLAGS, AJ_AUTH_CALL_TIMEOUT);
...
AJ_Status AJ_PeerHandleKeyExchange(AJ_Message *msg, AJ_Message *reply) {
  ...
  AJ_InfoPrintf(
      ("AJ_PeerHandleKeyExchange(msg=0x%p, reply=0x%p)\n", msg, reply));
  ...
  status = SetCipherSuite(msg->bus, suite);
  if (AJ_OK != status) {
    AJ_InfoPrintf(("AJ_PeerHandleKeyExchange(msg=0x%p, reply=0x%p): Suite 0x%x "
                   "not available\n",
                   msg, reply, suite));
    HandshakeComplete(AJ_ERR_SECURITY);
    return AJ_MarshalErrorMsg(msg, reply, AJ_ErrSecurityViolation);
  ...
  if (!handshakeContext.keyexchange) {
    AJ_WarnPrintf(("AJ_PeerHandleKeyExchange(msg=0x%p, reply=0x%p): Invalid "
                   "key exchange\n",
                   msg, reply));
    HandshakeComplete(AJ_ERR_SECURITY);
    return AJ_MarshalErrorMsg(msg, reply, AJ_ErrSecurityViolation);
  ...
  status = handshakeContext.keyexchange->Init(&handshakeContext.hash);
  if (AJ_OK != status) {
    AJ_WarnPrintf(("AJ_PeerHandleKeyExchange(msg=0x%p, reply=0x%p): Key "
                   "exchange init error\n",
                   msg, reply));
    HandshakeComplete(AJ_ERR_SECURITY);
    return AJ_MarshalErrorMsg(msg, reply, AJ_ErrSecurityViolation);
  ...
  if (AJ_OK != status) {
    AJ_InfoPrintf(("AJ_PeerHandleKeyExchange(msg=0x%p, reply=0x%p): Key "
  ...
  if (AJ_OK != status) {
    AJ_WarnPrintf(("AJ_PeerHandleKeyExchange(msg=0x%p, reply=0x%p): Compute "
                   "master secret error\n",
                   msg, reply));
    HandshakeComplete(AJ_ERR_SECURITY);
    return AJ_MarshalErrorMsg(msg, reply, AJ_ErrSecurityViolation);
  ...
  if (AJ_OK != status) {
    AJ_WarnPrintf(
        ("AJ_PeerHandleKeyExchange(msg=0x%p, reply=0x%p): Marshal error\n", msg,
         reply));
    HandshakeComplete(AJ_ERR_SECURITY);
    return AJ_MarshalErrorMsg(msg, reply, AJ_ErrSecurityViolation);
  }
...
AJ_Status AJ_PeerHandleKeyExchangeReply(AJ_Message *msg) {
  ...
  if (!handshakeContext.keyexchange) {
    AJ_WarnPrintf(
        ("AJ_PeerHandleKeyExchangeReply(msg=0x%p): Invalid key exchange\n",
         msg));
    HandshakeComplete(AJ_ERR_SECURITY);
    return AJ_ERR_SECURITY;
  }
  ...
  if (AJ_OK != status) {
    AJ_WarnPrintf(
        ("AJ_PeerHandleKeyExchangeReply(msg=0x%p): Unmarshal error\n", msg));
    HandshakeComplete(AJ_ERR_SECURITY);
    return AJ_ERR_SECURITY;
  }
  HostU32ToBigEndianU8(&suite, sizeof(suite), suiteb8);
  ...
  status = handshakeContext.keyexchange->Unmarshal(msg, &secret, &secretlen);
  if (AJ_OK != status) {
    AJ_WarnPrintf(("AJ_PeerHandleKeyExchangeReply(msg=0x%p): Key exchange "
                   "unmarshal error\n",
                   msg));
    HandshakeComplete(AJ_ERR_SECURITY);
    return AJ_ERR_SECURITY;
  ...
  status = ComputeMasterSecret(secret, secretlen);
  if (AJ_OK != status) {
    AJ_WarnPrintf(("AJ_PeerHandleKeyExchangeReply(msg=0x%p): Compute master "
                   "secret error\n",
                   msg));
    HandshakeComplete(AJ_ERR_SECURITY);
    return AJ_ERR_SECURITY;
...
static AJ_Status KeyAuthentication(AJ_Message *msg) {
  ...
  if (!handshakeContext.keyauthentication) {
    AJ_WarnPrintf(
        ("AJ_KeyAuthentication(msg=0x%p): Invalid key authentication\n", msg));
    HandshakeComplete(AJ_ERR_SECURITY);
    return AJ_ERR_SECURITY;
  }
  status = handshakeContext.keyauthentication->Init(
      msg->bus->authListenerCallback, handshakeContext.mastersecret,
      AJ_MASTER_SECRET_LEN, &handshakeContext.hash);
  if (AJ_OK != status) {
    AJ_WarnPrintf(
        ("AJ_KeyAuthentication(msg=0x%p): Key authentication init error\n",
         msg));
    HandshakeComplete(AJ_ERR_SECURITY);
    return AJ_ERR_SECURITY;
  }
   ...
   */
  AJ_MarshalMethodCall(msg->bus, &call, AJ_METHOD_KEY_AUTHENTICATION,
                       msg->sender, 0, AJ_NO_FLAGS, AJ_AUTH_CALL_TIMEOUT);
  status = handshakeContext.keyauthentication->Marshal(&call, AUTH_CLIENT);
  if (AJ_OK != status) {
    AJ_WarnPrintf(
        ("AJ_KeyAuthentication(msg=0x%p): Key authentication marshal error\n",
         msg));
    HandshakeComplete(AJ_ERR_SECURITY);
    return AJ_ERR_SECURITY;
  }
...
AJ_Status AJ_PeerHandleKeyAuthentication(AJ_Message *msg, AJ_Message *reply) {
  ...
  AJ_InfoPrintf(
      ("AJ_PeerHandleKeyAuthentication(msg=0x%p, reply=0x%p)\n", msg, reply));
  ...
  if (AJ_AUTH_EXCHANGED != handshakeContext.state) {
    AJ_InfoPrintf((
        "AJ_PeerHandleKeyAuthentication(msg=0x%p, reply=0x%p): Invalid state\n",
        msg, reply));
    HandshakeComplete(AJ_ERR_SECURITY);
    return AJ_MarshalErrorMsg(msg, reply, AJ_ErrSecurityViolation);
  }
  if (!handshakeContext.keyauthentication) {
    AJ_WarnPrintf(("AJ_PeerHandleKeyAuthentication(msg=0x%p, reply=0x%p): "
  ...
  status = handshakeContext.keyauthentication->Init(
      ...
      AJ_MASTER_SECRET_LEN, &handshakeContext.hash);
  if (AJ_OK != status) {
    AJ_WarnPrintf(("AJ_PeerHandleKeyAuthentication(msg=0x%p, reply=0x%p): Key "
                   "authentication init error\n",
                   msg, reply));
    HandshakeComplete(AJ_ERR_SECURITY);
    return AJ_MarshalErrorMsg(msg, reply, AJ_ErrSecurityViolation);
  }
  ...
  if (AJ_OK != status) {
    AJ_InfoPrintf(("AJ_PeerHandleKeyAuthentication(msg=0x%p, reply=0x%p): Key "
...
AJ_Status AJ_PeerHandleKeyAuthenticationReply(AJ_Message *msg) {
  ...
  if (AJ_AUTH_EXCHANGED != handshakeContext.state) {
    AJ_WarnPrintf((
        "AJ_PeerHandleKeyAuthenticationReply(msg=0x%p): Invalid state\n", msg));
    HandshakeComplete(AJ_ERR_SECURITY);
    return AJ_ERR_SECURITY;
  }
  if (!handshakeContext.keyauthentication) {
    AJ_WarnPrintf(("AJ_PeerHandleKeyAuthenticationReply(msg=0x%p): Invalid key "
                   "authentication\n",
                   msg));
    HandshakeComplete(AJ_ERR_SECURITY);
    return AJ_ERR_SECURITY;
  ...
  if (AJ_OK != status) {
    AJ_WarnPrintf(("AJ_PeerHandleKeyAuthenticationReply(msg=0x%p): Key "
                   "authentication unmarshal error\n",
                   msg));
    HandshakeComplete(AJ_ERR_SECURITY);
    return AJ_ERR_SECURITY;
  ...
  if (expiration) {
    status = SaveMasterSecret(peerGuid, expiration);
    if (AJ_OK != status) {
      AJ_WarnPrintf(("AJ_PeerHandleKeyAuthenticationReply(msg=0x%p): Save "
                     "master secret error\n",
                     msg));
    }
  }
...
static AJ_Status GenSessionKey(AJ_Message *msg) {
  ...
  AJ_MarshalMethodCall(msg->bus, &call, AJ_METHOD_GEN_SESSION_KEY, msg->sender,
                       0, AJ_NO_FLAGS, AJ_CALL_TIMEOUT);
  /*
...
AJ_Status AJ_PeerHandleGenSessionKey(AJ_Message *msg, AJ_Message *reply) {
  ...
  AJ_InfoPrintf(
      ("AJ_PeerHandleGenSessionKey(msg=0x%p, reply=0x%p)\n", msg, reply));
  ...
  if (AJ_AUTH_SUCCESS != handshakeContext.state) {
     ...
     */
    AJ_InfoPrintf((
        "AJ_PeerHandleGenSessionKey(msg=0x%p, reply=0x%p): Key not available\n",
        msg, reply));
  ...
  AJ_RandHex(sessionContext.nonce, sizeof(sessionContext.nonce), AJ_NONCE_LEN);
  status = KeyGen(msg->sender, AJ_ROLE_KEY_RESPONDER, nonce,
                  sessionContext.nonce, (uint8_t *)verifier, sizeof(verifier));
  if (status == AJ_OK) {
    AJ_MarshalReplyMsg(msg, reply);
...
AJ_Status AJ_PeerHandleGenSessionKeyReply(AJ_Message *msg) {
  ...
  if (msg->hdr->msgType == AJ_MSG_ERROR) {
    ...
    if (0 == strncmp(msg->x2.error, AJ_ErrResources, sizeof(AJ_ErrResources))) {
      status = AJ_ERR_RESOURCES;
               ...
               strncmp(msg->x2.error, AJ_ErrRejected, sizeof(AJ_ErrRejected))) {
      status = ExchangeSuites(msg);
    } else {
      status = AJ_ERR_SECURITY;
      HandshakeComplete(status);
  ...
  status = KeyGen(msg->sender, AJ_ROLE_KEY_INITIATOR, sessionContext.nonce,
                  nonce, (uint8_t *)verifier, sizeof(verifier));
  if (status == AJ_OK) {
    /*
  ...
  if (status == AJ_OK) {
     ...
     */
    AJ_MarshalMethodCall(msg->bus, &call, AJ_METHOD_EXCHANGE_GROUP_KEYS,
                         msg->sender, 0, AJ_FLAG_ENCRYPTED, AJ_CALL_TIMEOUT);
    AJ_GetGroupKey(NULL, groupKey);
    AJ_MarshalArg(&call, AJ_InitArg(&key, AJ_ARG_BYTE, AJ_ARRAY_FLAG, groupKey,
                                    sizeof(groupKey)));
...
AJ_Status AJ_PeerHandleExchangeGroupKeys(AJ_Message *msg, AJ_Message *reply) {
  ...
  AJ_InfoPrintf(
      ("AJ_PeerHandleExchangeGroupKeys(msg=0x%p, reply=0x%p)\n", msg, reply));
  ...
  if (status == AJ_OK) {
    ...
    AJ_MarshalReplyMsg(msg, reply);
    AJ_GetGroupKey(NULL, groupKey);
    status = AJ_MarshalArg(reply, AJ_InitArg(&key, AJ_ARG_BYTE, AJ_ARRAY_FLAG,
                                             groupKey, sizeof(groupKey)));
  } else {
    status = AJ_MarshalErrorMsg(msg, reply, AJ_ErrSecurityViolation);
  }
  HandshakeComplete(status);
...
AJ_Status AJ_PeerHandleExchangeGroupKeysReply(AJ_Message *msg) {
  ...
  if (arg.len != AES_KEY_LEN) {
    AJ_WarnPrintf(
        ("AJ_PeerHandleExchangeGroupKeysReply(msg=0x%p): AJ_ERR_INVALID\n",
         msg));
    status = AJ_ERR_INVALID;
  } else {
