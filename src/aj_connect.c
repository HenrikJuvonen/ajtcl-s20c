...
uint8_t AJ_GetMinProtoVersion() { return minProtoVersion; }
void AJ_SetMinProtoVersion(uint8_t min) { minProtoVersion = min; }

void SetBusAuthPwdCallback(BusAuthPwdFunc callback) {
  /*
   * This functionality is no longer provided but the function is still defined
   * for backwards
   * compatibility.
   */
}
...
static AJ_Status SendHello(AJ_BusAttachment *bus) {
  ...
  status = AJ_MarshalMethodCall(bus, &msg, AJ_METHOD_HELLO, AJ_DBusDestination,
                                0, AJ_FLAG_ALLOW_REMOTE_MSG, 5000);
  if (status == AJ_OK) {
...
static AJ_Status ReadLine(AJ_IOBuffer *rxBuf) {
  /*
   * All the authentication messages end in a CR/LF so read until we get a
   * newline
   */
  AJ_Status status = AJ_OK;
  while ((AJ_IO_BUF_AVAIL(rxBuf) == 0) || (*(rxBuf->writePtr - 1) != '\n')) {
    status = rxBuf->x1.recv(rxBuf, AJ_IO_BUF_SPACE(rxBuf), 3500);
    if (status != AJ_OK) {
      break;
    }
  }
...
static AJ_Status WriteLine(AJ_IOBuffer *txBuf, char *line) {
  ...
  return txBuf->x1.send(txBuf);
}
uint8_t AJ_GetRoutingProtoVersion(void) { return routingProtoVersion; }
/**
...
static AJ_Status AnonymousAuthAdvance(AJ_IOBuffer *rxBuf, AJ_IOBuffer *txBuf) {
  ...
  if (status == AJ_OK) {
    ...
    status = ReadLine(rxBuf);
    if (status == AJ_OK) {
      if (memcmp(rxBuf->readPtr, "INFORM_PROTO_VERSION",
                 strlen("INFORM_PROTO_VERSION")) != 0) {
        return AJ_ERR_ACCESS_ROUTING_NODE;
      }
      routingProtoVersion = atoi(
          (const char *)(rxBuf->readPtr + strlen("INFORM_PROTO_VERSION") + 1));
...
AJ_Status AJ_Authenticate(AJ_BusAttachment *bus) {
  ...
  status = bus->sock.tx.x1.send(&bus->sock.tx);
  if (status != AJ_OK) {
...
AJ_Status AJ_Connect(AJ_BusAttachment *bus, const char *serviceName,
  ...
  if (status != AJ_OK) {
    AJ_InfoPrintf(
        ("AJ_Connect(): AJ_Discover status=%s\n", AJ_StatusText(status)));
    goto ExitConnect;
  }
  ...
  if (status != AJ_OK) {
    AJ_InfoPrintf(
        ("AJ_Connect(): AJ_Serial_Up status=%s\n", AJ_StatusText(status)));
  }
#else
  status = AJ_Discover(serviceName, &service, timeout);
  if (status != AJ_OK) {
...
#endif
  status = AJ_Net_Connect(&bus->sock, service.ipv4port,
                          service.addrTypes & AJ_ADDR_IPV4, &service.ipv4);
  if (status != AJ_OK) {
    AJ_InfoPrintf(
        ("AJ_Connect(): AJ_Net_Connect status=%s\n", AJ_StatusText(status)));
    goto ExitConnect;
  }
  ...
  do {
    AJ_StateMachine();
    AJ_InitTimer(&now);
  } while (AJ_SerialLinkParams.linkState != AJ_LINK_ACTIVE &&
           AJ_GetTimeDifference(&now, &start) < timeout);

  if (AJ_SerialLinkParams.linkState != AJ_LINK_ACTIVE) {
    AJ_InfoPrintf(
        ("Failed to establish active SLAP connection in %u msec\n", timeout));
    AJ_SerialShutdown();
  ...
  if (status != AJ_OK) {
    AJ_InfoPrintf(
        ("AJ_Connect(): AJ_Authenticate status=%s\n", AJ_StatusText(status)));
    goto ExitConnect;
  }

  // subscribe to the signal NameOwnerChanged and wait for the response
  status = AJ_BusSetSignalRule(bus, "type='signal',member='NameOwnerChanged',"
                                    "interface='org.freedesktop.DBus'",
                               AJ_BUS_SIGNAL_ALLOW);
