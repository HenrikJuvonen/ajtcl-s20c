...
static AJ_Status ECDHE_Unmarshal(AJ_Message *msg, uint8_t **secret,
                                 size_t *secretlen);

AJ_KeyExchange AJ_KeyExchangeECDHE = {ECDHE_Init, ECDHE_Marshal,
                                      ECDHE_Unmarshal};
...
static AJ_Status ECDHE_Marshal(AJ_Message *msg) {
  ...
  if (AJ_OK != status) {
    AJ_ErrPrintf(
        ("AJ_ECDHE_Marshal(msg=0x%p): Marshal key material error\n", msg));
    return status;
  }
...
static AJ_Status ECDHE_Unmarshal(AJ_Message *msg, uint8_t **secret,
  ...
  if (AJ_OK != status) {
    AJ_InfoPrintf(
        ("AJ_ECDHE_Unmarshal(msg=0x%p): Unmarshal variant error\n", msg));
    return status;
  }
  ...
  if (AJ_OK != status) {
    AJ_InfoPrintf(
        ("AJ_ECDHE_Unmarshal(msg=0x%p): Unmarshal key material error\n", msg));
    return status;
  }
  if (1 + sizeof(ecc_publickey) != datalen) {
    AJ_InfoPrintf(
        ("AJ_ECDHE_Unmarshal(msg=0x%p): Invalid key material\n", msg));
    return AJ_ERR_SECURITY;
  }
  ...
  if (AJ_OK != status) {
    AJ_InfoPrintf(
        ("AJ_ECDHE_Unmarshal(msg=0x%p): Generate secret error\n", msg));
    return status;
  }
