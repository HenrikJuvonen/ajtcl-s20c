...
static AJ_Status ComputeVerifier(const char *label, uint8_t *buffer,
                                 size_t bufferlen);

#ifdef AUTH_ECDSA
static AJ_Status ECDSA_Init(AJ_AuthListenerFunc authlistener,
                            const uint8_t *mastersecret, size_t mastersecretlen,
...
AJ_KeyAuthentication AJ_KeyAuthenticationECDSA = {ECDSA_Init, ECDSA_Marshal,
                                                  ECDSA_Unmarshal, ECDSA_Final};
...
#ifdef AUTH_PSK
static AJ_Status PSK_Init(AJ_AuthListenerFunc authlistener,
                          const uint8_t *mastersecret, size_t mastersecretlen,
                          AJ_SHA256_Context *hash);
...
AJ_KeyAuthentication AJ_KeyAuthenticationPSK = {PSK_Init, PSK_Marshal,
                                                PSK_Unmarshal, PSK_Final};
...
#ifdef AUTH_NULL
static AJ_Status NULL_Init(AJ_AuthListenerFunc authlistener,
                           const uint8_t *mastersecret, size_t mastersecretlen,
                           AJ_SHA256_Context *hash);
...
AJ_KeyAuthentication AJ_KeyAuthenticationNULL = {NULL_Init, NULL_Marshal,
                                                 NULL_Unmarshal, NULL_Final};
#endif
...
static AJ_Status ECDSA_Init(AJ_AuthListenerFunc authlistener,
   ...
   */
  status =
      AJ_GetLocalCredential(AJ_CRED_TYPE_DSA_PRIVATE, DSA_PRV_KEY_ID, &prvcred);
  if (AJ_OK == status) {
    ...
    status = AJ_GetLocalCredential(AJ_CRED_TYPE_DSA_PUBLIC, DSA_PUB_KEY_ID,
                                   &prvcred);
  ...
  if (AJ_OK != status) {
     ...
     */
    status = AJ_StoreLocalCredential(AJ_CRED_TYPE_DSA_PRIVATE, DSA_PRV_KEY_ID,
                                     (uint8_t *)&ecdsactx.prv_key,
                                     sizeof(ecc_privatekey), kactx.expiration);
    if (AJ_OK != status) {
      AJ_WarnPrintf(("AJ_ECDSA_Init(): Store local credential error\n"));
      return AJ_ERR_SECURITY;
    }
    status = AJ_StoreLocalCredential(AJ_CRED_TYPE_DSA_PUBLIC, DSA_PUB_KEY_ID,
                                     (uint8_t *)&ecdsactx.pub_key,
                                     sizeof(ecc_publickey), kactx.expiration);
    if (AJ_OK != status) {
      AJ_WarnPrintf(("AJ_ECDSA_Init(): Store local credential error\n"));
      return AJ_ERR_SECURITY;
    }
...
static AJ_Status ECDSA_MarshalCertificate(AJ_Message *msg,
  ...
  if (certificate->version > MAX_NUM_CERTIFICATES) {
    AJ_WarnPrintf(
        ("AJ_ECDSA_MarshalCertificate(msg=0x%p): Invalid certificate version\n",
         msg));
    return AJ_ERR_SECURITY;
  }
  ...
  if (!b8) {
    AJ_WarnPrintf(
        ("AJ_ECDSA_MarshalCertificate(msg=0x%p): AJ_ERR_RESOURCS\n", msg));
    return AJ_ERR_RESOURCES;
  }
  status = AJ_BigEndianEncodeCertificate(certificate, b8, certificate->size);
  if (AJ_OK != status) {
    AJ_WarnPrintf(
        ("AJ_ECDSA_MarshalCertificate(msg=0x%p): Encode certificate error\n",
         msg));
...
static AJ_Status ECDSA_Marshal(AJ_Message *msg, uint8_t role) {
  ...
  if (AJ_OK != status) {
    AJ_WarnPrintf(
        ("AJ_ECDSA_Marshal(msg=0x%p): Compute verifier error\n", msg));
    return AJ_ERR_SECURITY;
  }
  ...
  status = AJ_CreateCertificate(certificate, 0, &ecdsactx.pub_key, NULL, NULL,
                                verifier, 0);
  if (AJ_OK != status) {
    AJ_WarnPrintf(
        ("AJ_ECDSA_Marshal(msg=0x%p): Create certificate error\n", msg));
    goto Exit;
  }
  status = AJ_SignCertificate(certificate, &ecdsactx.prv_key);
  if (AJ_OK != status) {
    AJ_WarnPrintf(
        ("AJ_ECDSA_Marshal(msg=0x%p): Sign certificate error\n", msg));
    goto Exit;
  }
  ...
  if (AJ_OK != status) {
    AJ_WarnPrintf(
        ("AJ_ECDSA_Marshal(msg=0x%p): Marshal certificate error\n", msg));
    goto Exit;
  }
  ...
  while (chainlen > 0) {
    if (chainlen < sizeof(AJ_Certificate)) {
      AJ_WarnPrintf(
          ("AJ_ECDSA_Marshal(msg=0x%p): Invalid certificate chain\n", msg));
      status = AJ_ERR_SECURITY;
      goto Exit;
    }
...
static AJ_Status ECDSA_UnmarshalCertificate(AJ_Message *msg,
                                            AJ_Certificate *certificate,
                                            uint8_t role) {
  AJ_Status status;
  AJ_Arg struct1;
  ...
  if (AJ_OK != status) {
    if (AJ_ERR_NO_MORE != status) {
      AJ_WarnPrintf(("AJ_ECDSA_UnmarshalCertificate(msg=0x%p): Unmarshal "
                     "container error\n",
                     msg));
    }
    return status;
  ...
  if (AJ_OK != status) {
    AJ_WarnPrintf(
        ("AJ_ECDSA_UnmarshalCertificate(msg=0x%p): Unmarshal error\n", msg));
    return AJ_ERR_SECURITY;
  }
  status = AJ_BigEndianDecodeCertificate(certificate, b8, b8len);
  if (AJ_OK != status) {
    AJ_WarnPrintf(
        ("AJ_ECDSA_UnmarshalCertificate(msg=0x%p): Decode certificate error\n",
         msg));
    return AJ_ERR_SECURITY;
  }
  if (certificate->version > MAX_NUM_CERTIFICATES) {
    AJ_WarnPrintf(("AJ_ECDSA_UnmarshalCertificate(msg=0x%p): Invalid "
                   "certificate version\n",
                   msg));
    return AJ_ERR_SECURITY;
  }
  ...
  if (AJ_OK != status) {
    AJ_WarnPrintf(
        ("AJ_ECDSA_UnmarshalCertificate(msg=0x%p): Unmarshal container error\n",
         msg));
    return AJ_ERR_SECURITY;
  }
...
static AJ_Status ECDSA_Unmarshal(AJ_Message *msg, uint8_t role) {
  ...
  if (AJ_OK != status) {
    AJ_WarnPrintf(
        ("AJ_ECDSA_Unmarshal(msg=0x%p): Compute verifier error\n", msg));
    return AJ_ERR_SECURITY;
  }
  ...
  if (AJ_OK != status) {
    AJ_InfoPrintf(
        ("AJ_ECDSA_Unmarshal(msg=0x%p): Unmarshal variant error\n", msg));
    return AJ_ERR_SECURITY;
  }
  ...
  if (AJ_OK != status) {
    AJ_InfoPrintf(
        ("AJ_ECDSA_Unmarshal(msg=0x%p): Unmarshal container error\n", msg));
    return AJ_ERR_SECURITY;
  }
  ...
  if (AJ_OK != status) {
    AJ_InfoPrintf(
        ("AJ_ECDSA_Unmarshal(msg=0x%p): Unmarshal certificate error\n", msg));
    goto Exit;
  }
  /*
  ...
  if (0 != certificate->version) {
    AJ_InfoPrintf(
        ("AJ_ECDSA_Unmarshal(msg=0x%p): Invalid certificate version %X\n", msg,
         certificate->version));
    status = AJ_ERR_SECURITY;
    goto Exit;
  }
  ...
  while (AJ_OK == status) {
    ...
    if (!trusted) {
      ...
      if (!b8) {
        AJ_InfoPrintf(
            ("AJ_ECDSA_Unmarshal(msg=0x%p): AJ_ERR_RESOURCES\n", msg));
      ...
      status = kactx.authlistener(AUTH_SUITE_ECDHE_ECDSA, AJ_CRED_CERT_TRUST,
                                  &credential);
      if (AJ_OK == status) {
        trusted = 1;
      }
    ...
    if (!isfirst && !delegate) {
      AJ_InfoPrintf(
          ("AJ_ECDSA_Unmarshal(msg=0x%p): Certificate delegation invalid\n",
           msg));
      status = AJ_ERR_SECURITY;
      goto Exit;
    }
    if (0 != memcmp(&issuer, &certificate->subject, sizeof(ecc_publickey))) {
      AJ_InfoPrintf((
          "AJ_ECDSA_Unmarshal(msg=0x%p): Certificate chaining invalid\n", msg));
      status = AJ_ERR_SECURITY;
      goto Exit;
    }
    ...
    if (AJ_OK != status) {
      AJ_InfoPrintf(
          ("AJ_ECDSA_Unmarshal(msg=0x%p): Invalid certificate\n", msg));
      goto Exit;
    }
    isfirst = 0;
    memcpy(&issuer, &certificate->issuer, sizeof(ecc_publickey));
  ...
  if (trusted) {
    ...
    if (AJ_OK == status) {
      ...
      kactx.authlistener(AUTH_SUITE_ECDHE_ECDSA, AJ_CRED_CERT_ROOT,
                         &credential);
    }
    AJ_Free(b8);
  } else {
    AJ_InfoPrintf((
        "AJ_ECDSA_Unmarshal(msg=0x%p): Certificate issuer not trusted\n", msg));
    status = AJ_ERR_SECURITY;
  }
...
static AJ_Status PSK_Marshal(AJ_Message *msg, uint8_t role) {
  ...
  status = AJ_MarshalArgs(msg, "(ayay)", pskctx.hint, pskctx.hintlen, verifier,
                          sizeof(verifier));
...
static AJ_Status PSK_Unmarshal(AJ_Message *msg, uint8_t role) {
  ...
  if (AJ_OK != status) {
    AJ_InfoPrintf(
        ("AJ_PSK_Unmarshal(msg=0x%p): Unmarshal variant error\n", msg));
    return AJ_ERR_SECURITY;
  }
  ...
  status = AJ_UnmarshalArgs(msg, "(ayay)", &remotehint, &remotehintlen,
                            &remotesig, &remotesiglen);
  if (AJ_OK != status) {
  ...
  if (AUTH_VERIFIER_LEN != remotesiglen) {
    AJ_InfoPrintf(
        ("AJ_PSK_Unmarshal(msg=0x%p): Invalid signature size\n", msg));
    return AJ_ERR_SECURITY;
  }
  ...
  if (AJ_OK != status) {
    AJ_WarnPrintf(
        ("AJ_PSK_Unmarshal(msg=0x%p): Compute verifier error\n", msg));
    return AJ_ERR_SECURITY;
  }
...
static AJ_Status NULL_Unmarshal(AJ_Message *msg, uint8_t role) {
  ...
  if (AJ_OK != status) {
    AJ_WarnPrintf(
        ("AJ_NULL_Unmarshal(msg=0x%p): Compute verifier error\n", msg));
    return AJ_ERR_SECURITY;
  }
  ...
  if (AJ_OK != status) {
    AJ_InfoPrintf(
        ("AJ_NULL_Unmarshal(msg=0x%p): Unmarshal variant error\n", msg));
    return AJ_ERR_SECURITY;
  }
  ...
  if (AUTH_VERIFIER_LEN != remotesiglen) {
    AJ_InfoPrintf(
        ("AJ_NULL_Unmarshal(msg=0x%p): Invalid signature size\n", msg));
    return AJ_ERR_SECURITY;
  }
  if (0 != memcmp(verifier, remotesig, AUTH_VERIFIER_LEN)) {
