...
typedef struct _AJ_BusLinkWatcher {
  uint8_t numOfPingTimedOut; /**< Number of probe request packets already sent
                                but unacked */
  uint8_t linkTimerInited;   /**< If timer linkTimer is inited */
  uint8_t pingTimerInited;   /**< If timer pingTimer is inited */
  AJ_Time linkTimer; /**< Timer for tracking activities over the link to the
                        daemon bus */
  AJ_Time pingTimer; /**< Timer for tracking probe request packets */
} AJ_BusLinkWatcher;

static uint32_t
    busLinkTimeout; /**< Timeout value for the link to the daemon bus */
static AJ_BusLinkWatcher
    busLinkWatcher; /**< Data structure that maintains information for tracking
                       the link to the daemon bus */
...
AJ_Status AJ_SetBusLinkTimeout(AJ_BusAttachment *bus, uint32_t timeout) {
  ...
  timeout =
      (timeout > AJ_MIN_BUS_LINK_TIMEOUT) ? timeout : AJ_MIN_BUS_LINK_TIMEOUT;
      ...
      if (eclipse >= busLinkTimeout) {
        if (!busLinkWatcher.pingTimerInited) {
          ...
          if (AJ_OK != AJ_SendLinkProbeReq(bus)) {
            AJ_ErrPrintf(
                ("AJ_BusLinkStateProc(): AJ_SendLinkProbeReq() failure"));
          }
        } else {
...
AJ_Status AJ_SendLinkProbeReq(AJ_BusAttachment *bus) {
  ...
  status = AJ_MarshalSignal(bus, &msg, AJ_SIGNAL_PROBE_REQ, AJ_BusDestination,
                            0, 0, 0);
  if (status == AJ_OK) {
