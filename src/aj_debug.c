...
static const char *const msgType[] = {"INVALID", "CALL", "REPLY", "ERROR",
                                      "SIGNAL"};
...
void _AJ_DumpMsg(const char *tag, AJ_Message *msg, uint8_t body) {
  if (msg->hdr && _AJ_DbgHeader(AJ_DEBUG_ERROR, NULL, 0)) {
    ...
    AJ_AlwaysPrintf(("%s message[%d] type %s sig=\"%s\"\n", tag,
                     msg->hdr->serialNum,
                     msgType[(msg->hdr->msgType <= 4) ? msg->hdr->msgType : 0],
                     msg->signature));
    switch (msg->hdr->msgType) {
    case AJ_MSG_SIGNAL:
    case AJ_MSG_METHOD_CALL:
      AJ_AlwaysPrintf(("        %s::%s\n", msg->iface, msg->x2.member));
      break;
...
int _AJ_DbgHeader(AJ_DebugLevel level, const char *file, int line) {
  ...
  if (level <= AJ_DbgLevel) {
    ...
    if (file) {
      ...
      AJ_AlwaysPrintf(
          ("%03d.%03d %s:%d ", msec / 1000, msec % 1000, file, line));
    } else {
