...
static AJ_Status ParseIsAt(AJ_IOBuffer *rxBuf, const char *prefix,
                           AJ_Service *service) {
...
/*
 * Function for determining whether or not to send a WHO-HAS packet. The
 * algorithm
 * will send WHO-HAS packets every second for AJ_DISCOVER_ATTEMPTS seconds, then
 * wait
 ...
 * AJ_THIRD_BACKOFF_TIME seconds until the timeout passed into AJ_Discover()
 * expires
 */
static uint8_t sendDiscovery(uint32_t discoverAttempts,
...
AJ_Status AJ_Discover(const char *prefix, AJ_Service *service,
  ...
  while ((int32_t)timeout > 0) {
    if (sendDiscovery(discoverAttempts, &discoverTimer)) {
      ...
      status = sock.tx.x1.send(&sock.tx);
      AJ_InfoPrintf(("AJ_Discover(): status=%s\n", AJ_StatusText(status)));
      /*
    ...
    while (TRUE) {
      ...
      status = sock.rx.x1.recv(&sock.rx, AJ_IO_BUF_SPACE(&sock.rx),
                               AJ_WHO_HAS_TIMEOUT);
      if (status != AJ_OK) {
        /*
