...
/**
 * controls rate at which we send synch packets when the link is down in
 * milliseconds
 */
const uint32_t CONN_TIMEOUT = 200;

/**
 * controls how long we will wait for a confirmation packet after we synchronize
 * in milliseconds
 */
const uint32_t NEGO_TIMEOUT = 200;

/**
 * controls how long we will send disconnect packet when the application is
 * closing the link in milliseconds
 */
...
static void AdjustTimeoutValues(uint32_t packetSize) {
  // one start bit, eight data bits, one parity, and one stop bit equals eleven
  // bits sent per byte
  // acknowledgement packets should be sent within twice the time to send one
  // packet
  // resends should be sent shortly after three times the packet delivery time
  AJ_SerialLinkParams.txAckTimeout =
      (packetSize * 11 * 1000 * 2) / AJ_SerialLinkParams.bitRate;
  AJ_SerialLinkParams.txResendTimeout =
      (packetSize * 11 * 1000 * 3) / AJ_SerialLinkParams.bitRate;
...
static void ProcessNegoPacket(const uint8_t *buffer) {
  ...
  AJ_SerialLinkParams.packetSize =
      min(AJ_SerialLinkParams.packetSize, max_payload);

  proto_version = (buffer[6] >> 2) & 0x003F;
  AJ_AlwaysPrintf(("Read protocol version: %u\n", proto_version));
  AJ_SerialLinkParams.protoVersion =
      min(AJ_SerialLinkParams.protoVersion, proto_version);
  ...
  AJ_SerialLinkParams.windowSize =
      min(AJ_SerialLinkParams.maxWindowSize, window_size);
}
...
static void SendNegotiationPacket(const char *pkt_type) {
  ...
  NegotiationPacket[6] =
      (AJ_SerialLinkParams.protoVersion << 2) | encoded_window_size;
  AJ_SerialTX_EnqueueCtrl(NegotiationPacket, sizeof(NegotiationPacket),
                          AJ_SERIAL_CTRL);
}
...
static void SendLinkPacket() {
  ...
  case AJ_LINK_UNINITIALIZED:
     ...
     * Send a sync packet.
     */
    AJ_SerialTX_EnqueueCtrl((uint8_t *)ConnPkt, sizeof(ConnPkt),
                            AJ_SERIAL_CTRL);
  ...
  case AJ_LINK_DYING:
    ...
    if (AJ_SerialLinkParams.protoVersion >= SLAP_VERSION_DISCONNECT_FEATURE) {
      AJ_SerialTX_EnqueueCtrl((uint8_t *)DiscPkt, sizeof(DiscPkt),
                              AJ_SERIAL_CTRL);
...
void AJ_Serial_LinkPacket(uint8_t *buffer, uint16_t len) {
  ...
  case AJ_LINK_UNINITIALIZED:
     ...
     */
    if (pktType == CONN_PKT) {
      AJ_SerialTX_EnqueueCtrl((uint8_t *)AcptPkt, sizeof(AcptPkt),
                              AJ_SERIAL_CTRL);
  ...
  case AJ_LINK_ACTIVE:
    ...
    if (pktType == NEGO_PKT) {
      ...
      // got a connection after active, so the link must have gone down without
      // our knowledge
      AJ_WarnPrintf(
          ("Received CONN while in active state - Moving to LINK_DEAD\n"));
      AJ_SerialLinkParams.linkState = AJ_LINK_DEAD;
    } else if (pktType == DISC_PKT) {
      // received a disconnect packet, move to link dying state.
      AJ_WarnPrintf((
          "Received DISC while in active state - other side is going away.\n"));
      AJ_SerialTX_EnqueueCtrl((uint8_t *)DrspPkt, sizeof(DrspPkt),
                              AJ_SERIAL_CTRL);
  ...
  case AJ_LINK_DYING:
    if (pktType == DISC_PKT) {
      // simultaneous DISC while in dying state, send a DRSP to cleanly shutdown
      // the other side.
      ...
      AJ_WarnPrintf(("Received DRSP while in dying state - other side knows we "
                     "are going away.\n"));
      AJ_SerialLinkParams.linkState = AJ_LINK_DEAD;
    }
    break;
...
AJ_Status AJ_SerialInit(const char *ttyName, uint32_t bitRate,
                        uint8_t windowSize, uint16_t packetSize) {
  AJ_Status status;
...
void AJ_SerialDisconnect(void) {
  ...
  if (AJ_SerialLinkParams.linkState != AJ_LINK_DEAD) {
    ...
    do {
      AJ_StateMachine();
      AJ_InitTimer(&now);
    } while (AJ_SerialLinkParams.linkState != AJ_LINK_DEAD &&
             AJ_GetTimeDifference(&now, &start) < DISC_TIMEOUT * 4);
