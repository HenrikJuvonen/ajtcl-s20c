...
static AJ_Status ReadRemainderOfCredential(AJ_NV_DATASET *handle,
                                           AJ_PeerCred *cred) {
  size_t size, toRead;
  ...
  if (toRead != size) {
    AJ_ErrPrintf(
        ("ReadRemainderOfCredential(): AJ_ERR_FAILURE on read failure\n"));
    return AJ_ERR_FAILURE;
  }
  if (cred->associationLen > 0) {
    cred->association = (uint8_t *)AJ_Malloc(cred->associationLen);
    if (!cred->association) {
      AJ_ErrPrintf(("ReadRemainderOfCredential(): AJ_ERR_FAILURE on memory "
                    "allocation failure of size %d\n",
                    cred->associationLen));
      return AJ_ERR_FAILURE;
    }
    /* Read association */
    ...
    if (toRead != size) {
      AJ_ErrPrintf(("ReadRemainderOfCredential(): AJ_ERR_FAILURE on read "
                    "failure on field association\n"));
      return AJ_ERR_FAILURE;
    }
  }
  /* Read data length */
...
static uint16_t FindCredential(const uint16_t credType, const uint8_t *credId,
                               uint8_t credIdLen, AJ_PeerCred **credHolder) {
  AJ_Status status;
  ...
  for (; slot < AJ_CREDS_NV_ID_END; slot++) {
    if (AJ_NVRAM_Exist(slot)) {
      ...
      if (!handle) {
        AJ_ErrPrintf(
            ("FindCredential(): fail to open data set with slot = %d\n", slot));
        continue;
      }
      /* Read type */
      if (sizeof(localCredType) !=
          AJ_NVRAM_Read(&localCredType, sizeof(localCredType), handle)) {
        AJ_ErrPrintf(("FindCredential(): fail to read credential type %zu "
                      "bytes from data set with slot = %d\n",
      ...
      /* Read id length */
      if (sizeof(localCredIdLen) !=
          AJ_NVRAM_Read(&localCredIdLen, sizeof(localCredIdLen), handle)) {
        AJ_ErrPrintf(("FindCredential(): fail to read slot length %zu bytes "
                      "from data set with slot = %d\n",
      ...
      /* Read id */
      if (localCredIdLen !=
          AJ_NVRAM_Read(localCredId, localCredIdLen, handle)) {
        AJ_ErrPrintf(("FindCredential(): fail to read slot %u bytes from data "
                      "set with slot = %d\n",
                      localCredIdLen, slot));
        AJ_NVRAM_Close(handle);
        AJ_Free(localCredId);
...
AJ_Status AJ_GetCredential(const uint16_t credType, const uint8_t *id,
                           uint8_t idLen, AJ_PeerCred **customCredHolder) {
...
static size_t GetCredentialSize(AJ_PeerCred *cred) {
  ...
  return sizeof(cred->type) + sizeof(cred->idLen) + cred->idLen +
         sizeof(cred->expiration) + sizeof(cred->associationLen) +
         cred->associationLen + sizeof(cred->dataLen) + cred->dataLen;
}
...
static AJ_Status UpdateCred(AJ_PeerCred *aCred, uint16_t slot) {
  ...
  if (toWrite != size) {
    AJ_ErrPrintf(
        ("UpdateCred(): AJ_ERR_FAILURE on write failure on type field\n"));
    goto Exit;
  }
  ...
  if (toWrite != size) {
    AJ_ErrPrintf(
        ("UpdateCred(): AJ_ERR_FAILURE on write failure on idLen field\n"));
    goto Exit;
  }
  if (aCred->idLen > 0) {
    toWrite = aCred->idLen;
    ...
    if (toWrite != size) {
      AJ_ErrPrintf(
          ("UpdateCred(): AJ_ERR_FAILURE on write failure on id field\n"));
      goto Exit;
    }
  }
  /* Write expiration */
  ...
  if (toWrite != size) {
    AJ_ErrPrintf((
        "UpdateCred(): AJ_ERR_FAILURE on write failure on field expiration\n"));
    goto Exit;
  }
  ...
  if (toWrite != size) {
    AJ_ErrPrintf(("UpdateCred(): AJ_ERR_FAILURE on write failure on field "
                  "associationLen\n"));
    goto Exit;
  }
  ...
  if (toWrite != size) {
    AJ_ErrPrintf(
        ("UpdateCred(): AJ_ERR_FAILURE on write failure on field dataLen\n"));
    goto Exit;
  }
  if (aCred->dataLen > 0) {
    ...
    if (toWrite != size) {
      AJ_ErrPrintf(
          ("UpdateCred(): AJ_ERR_FAILURE on write failure on field data\n"));
      goto Exit;
    }
  }
 ...
 * @param[out] deleteSlot holder for the slot of deleted credential or zero if
 * not deleted
...
static AJ_Status AJ_DeleteOldestCredential(uint16_t *deleteSlot) {
  ...
  *deleteSlot = 0;
  for (; slot < AJ_CREDS_NV_ID_END; slot++) {
    ...
    if (!handle) {
      AJ_ErrPrintf((
          "AJ_DeleteOldestCredential(): fail to open data set with slot = %d\n",
          slot));
      continue;
    }
    /* Read type */
    if (sizeof(localCredType) !=
        AJ_NVRAM_Read(&localCredType, sizeof(localCredType), handle)) {
      AJ_ErrPrintf(("AJ_DeleteOldestCredential(): fail to read credential type "
                    "%zu bytes from data set with slot = %d\n",
    ...
    if (sizeof(localCredIdLen) !=
        AJ_NVRAM_Read(&localCredIdLen, sizeof(localCredIdLen), handle)) {
      AJ_ErrPrintf(("AJ_DeleteOldestCredential(): fail to read slot length %zu "
                    "bytes from data set with slot = %d\n",
                    sizeof(localCredIdLen), slot));
      AJ_NVRAM_Close(handle);
      continue;
    ...
    if (!localCredIdLen) {
      AJ_ErrPrintf((
          "AJ_DeleteOldestCredential(): id length is zero in slot %d\n", slot));
      AJ_NVRAM_Close(handle);
      continue;
    }
    localCredId = (uint8_t *)AJ_Malloc(localCredIdLen);
    ...
    if (localCredIdLen != AJ_NVRAM_Read(localCredId, localCredIdLen, handle)) {
      AJ_ErrPrintf(("AJ_DeleteOldestCredential(): fail to read slot %u bytes "
                    "from data set with slot = %d\n",
                    localCredIdLen, slot));
      AJ_NVRAM_Close(handle);
      AJ_Free(localCredId);
    ...
    if (status != AJ_OK) {
      AJ_ErrPrintf(
          ("AJ_DeleteOldestCredential(): AJ_ERR_FAILURE on read failure \n"));
      return status;
    }
...
AJ_Status AJ_StoreCredential(AJ_PeerCred *aCred) {
  ...
  if (!slot) {
    ...
    while ((AJ_OK == status) &&
           (!slot || (len >= AJ_NVRAM_GetSizeRemaining()))) {
      AJ_InfoPrintf(
          ("AJ_StoreCredential(aCred=0x%p): Remaining %d Required %d Slot %d\n",
...
AJ_Status AJ_StorePeerSecret(const AJ_GUID *peerGuid, const uint8_t *secret,
  ...
  AJ_InfoPrintf(("AJ_StorePeerSecret(peerGuid=0x%p, secret=0x%p, len=%d, "
                 "expiration=0x%08X)\n",
...
AJ_Status AJ_DeleteCredential(const uint16_t credType, const uint8_t *id,
                              uint8_t idLen) {
...
AJ_Status AJ_DeletePeerCredential(const AJ_GUID *peerGuid) {
  return AJ_DeleteCredential(AJ_CRED_TYPE_GENERIC, peerGuid->val,
                             sizeof(AJ_GUID));
}
...
AJ_Status AJ_GetPeerCredential(const AJ_GUID *peerGuid,
                               AJ_PeerCred **peerCredHolder) {
  return AJ_GetCredential(AJ_CRED_TYPE_GENERIC, peerGuid->val,
                          sizeof(peerGuid->val), peerCredHolder);
}
...
AJ_Status AJ_GetLocalGUID(AJ_GUID *localGuid) {
  ...
  if (AJ_NVRAM_Exist(AJ_LOCAL_GUID_NV_ID)) {
    ...
    if (handle) {
      if (sizeof(AJ_GUID) ==
          AJ_NVRAM_Read(localGuid, sizeof(AJ_GUID), handle)) {
        status = AJ_OK;
      } else {
        AJ_ErrPrintf(("AJ_GetLocalGUID(): fail to read slot length %zu bytes "
                      "from slot = %d\n",
    ...
    if (handle) {
      if (sizeof(AJ_GUID) ==
          AJ_NVRAM_Write(localGuid, sizeof(AJ_GUID), handle)) {
        status = AJ_OK;
      } else {
        AJ_ErrPrintf(("AJ_GetLocalGUID(): fail to write slot length %zu bytes "
                      "to slot = %d\n",
                      sizeof(AJ_GUID), AJ_LOCAL_GUID_NV_ID));
      }
      status = AJ_NVRAM_Close(handle);
...
AJ_Status AJ_StoreLocalCredential(const uint16_t credType, const uint16_t id,
                                  const uint8_t *data, const uint8_t len,
                                  uint32_t expiration) {
  AJ_PeerCred cred;

  AJ_InfoPrintf(("AJ_StoreLocalCredential(type=0x%04X, id=0x%p, data=0x%p, "
                 "len=%d, expiration=0x%08X)\n",
...
AJ_Status AJ_GetLocalCredential(const uint16_t credType, const uint16_t id,
                                AJ_PeerCred **credHolder) {
  return AJ_GetCredential(credType, (const uint8_t *)&id, sizeof(id),
                          credHolder);
}
