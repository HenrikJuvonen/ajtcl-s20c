...
/**
 * This enumerated type is for tracking the state of the receive packet as it is
 * being decoded.
 */
typedef enum { PACKET_NEW, PACKET_OPEN, PACKET_FLUSH, PACKET_ESCAPE } PKT_STATE;
...
AJ_Status AJ_SerialRX_Init(void) {
  ...
  for (i = 0; i < AJ_SerialLinkParams.maxWindowSize + 1; i++) {
    ...
    bufferRxFreeList->buffer =
        AJ_Malloc(SLIPPED_LEN(AJ_SerialLinkParams.packetSize));
    bufferRxFreeList->actualLen = 0;
    bufferRxFreeList->allocatedLen =
        SLIPPED_LEN(AJ_SerialLinkParams.packetSize);
...
AJ_Status AJ_SerialRecv(uint8_t *buffer, uint16_t req, uint32_t timeout,
                        uint16_t *recv) {
  ...
  do {
    ...
    while (RxRecv && len) {
      ...
      if (num == (pkt->len - 6)) { /// use pkt->len, because we might have eaten
                                   /// out of this buffer already
        // we used a full packet
        // put that packet back on the RxFreeList, decrement pendingRecv, and
        // send Ack.
        RxRecv = RxRecv->next;
        AJ_SerialReturnPacketToFreeList(pkt);
        AJ_DebugCheckPacketList(RxFreeList,
                                "RxFreeList serialrecv AFTER FULL PACKET");
        AJ_DebugCheckPacketList(RxRecv, "RxRecv serialrecv AFTER FULL PACKET");
        AJ_DebugCheckPacketList(RxPacket,
                                "RxFreeList serialrecv AFTER FULL PACKET");
        --pendingRecv;
...
/**
 * This function checks packet integrity and forwards good packets to the
 * appropriate
...
/**
 * This function is called from the physical transport layer when data is
 * received.
  ...
  while (bytes-- > 0) {
    ...
    case PACKET_OPEN:
      ...
      if (rx == BOUNDARY_BYTE) {
        ...
        CompletePacket();
        // the packet will be put back on the RxFreeList when the upper layer
        // has retrieved it.
        break;
      }
      if (rx == ESCAPE_BYTE) {
      ...
      if (RxPacket->len == maxRxFrameSize) {
        ...
        RxPacket->state = PACKET_NEW;
        AJ_AlwaysPrintf(
            ("AJ_SerialRx_Receive: Packet overrun %d\n", RxPacket->len));
        break;
      }
...
void AJ_ProcessRxBufferList() {
  ...
  while (bufferRxQueue && RxFreeList) {
    ...
    UART_RxComplete(currentSlippedBuffer->buffer,
                    currentSlippedBuffer->actualLen);
    AJ_PauseRX();

    if (pendingRecvBuffer == NULL) {
      // Free list was previously NULL, so re-enable reading
      // Save a pointer to the recv buffer, so we can keep track when the
      // AJ_RecieveCallback occurs.
