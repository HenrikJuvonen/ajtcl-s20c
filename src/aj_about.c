...
void AJ_AboutSetIcon(const uint8_t *data, uint16_t size, const char *mime,
                     const char *url) {
  icon.data = data;
 ...
 */
static AJ_Status AboutGetProp(AJ_Message *replyMsg, uint32_t propId,
                              void *context) {
...
static AJ_Status MarshalObjectDescriptions(AJ_Message *msg) {
   ...
   */
  for (obj = AJ_InitObjectIterator(&iter, AJ_OBJ_FLAG_ANNOUNCED |
                                              AJ_OBJ_FLAG_DESCRIBED,
                                   AJ_OBJ_FLAG_HIDDEN);
       obj != NULL; obj = AJ_NextObject(&iter)) {
    size_t i;
    AJ_Arg structure;
    AJ_Arg ifcList;
    const char *iface;
    ...
    /*
     * Add the AllSeenIntrospectableInterface if this object is flagged as being
     * described
     */
...
AJ_Status AJ_AboutAnnounce(AJ_BusAttachment *bus) {
  ...
  status = AJ_MarshalSignal(bus, &announcement, AJ_SIGNAL_ABOUT_ANNOUNCE, NULL,
                            0, ALLJOYN_FLAG_SESSIONLESS, 0);
  if (status != AJ_OK) {
    goto ErrorExit;
  }
...
static AJ_Status AboutIconGetProp(AJ_Message *replyMsg, uint32_t propId,
                                  void *context) {
...
void AJ_AboutSetShouldAnnounce() { doAnnounce = TRUE; }
