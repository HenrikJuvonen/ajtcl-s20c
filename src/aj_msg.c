...
static const uint8_t TypeForHdr[] = {
    AJ_ARG_INVALID,   AJ_ARG_OBJ_PATH, /* AJ_HDR_OBJ_PATH     */
    ...
    AJ_ARG_INVALID,   AJ_ARG_INVALID,  AJ_ARG_INVALID, AJ_ARG_INVALID,
    AJ_ARG_INVALID,   AJ_ARG_INVALID,  AJ_ARG_UINT32, /* AJ_HDR_TIMESTAMP */
 ...
 *
 * ((t) == '(' || (t) == ')') ? (t) - '('       --> If the value is ) or (, get
 * the value in TypeFlags
 * :
 * (((t) < 'a' || (t) > '}') ? '}' + 2 - 'a'    --> The value is too high or too
 * low, return TypeFlags[30] (0)
 * :
 * (t) + 2 - 'a'                                --> The value is valid, get the
 * value in TypeFlags
 */
#define TYPE_FLAG(t)                                                           \
  TypeFlags[((t) == '(' || (t) == ')')                                         \
...
int AJ_IsScalarType(char typeId) { return IsScalarType(typeId) != 0; }

int AJ_IsStringType(char typeId) {
  return (TYPE_FLAG(typeId) & AJ_STRING) != 0;
}

int AJ_IsBasicType(char typeId) { return IsBasicType(typeId) != 0; }

size_t AJ_GetTypeSize(char typeId) { return SizeOfType(typeId); }
...
static uint32_t PadForType(char typeId, AJ_IOBuffer *ioBuf) {
  uint8_t *base =
      (ioBuf->direction == AJ_IO_BUF_RX) ? ioBuf->readPtr : ioBuf->writePtr;
...
#define ENDSWAP32(v)                                                           \
  (((v) >> 24) | (((v)&0xFF0000) >> 8) | (((v)&0x00FF00) << 8) | ((v) << 24))

static void EndianSwap(AJ_Message *msg, uint8_t typeId, void *data,
                       uint32_t num) {
...
/*
 * Computes total size of a message - note header is padded to an 8 byte
 * boundary
 */
static uint32_t MessageLen(AJ_Message *msg) {
  return sizeof(AJ_MsgHeader) + ((msg->hdr->headerLen + 7) & 0xFFFFFFF8) +
         msg->hdr->bodyLen;
}
...
static AJ_Status DecryptMessage(AJ_Message *msg) {
  ...
  /*
   * Use the group key for multicast and broadcast signals the session key
   * otherwise.
   */
  ...
  if (status != AJ_OK) {
    ...
    status = AJ_Decrypt_CCM(key, ioBuf->bufStart, mlen - MAC_LENGTH, hLen,
                            MAC_LENGTH, nonce, sizeof(nonce));
    EndianSwap(msg, AJ_ARG_INT32, &msg->hdr->bodyLen, 3);
  }
  return status;
}
...
static AJ_Status EncryptMessage(AJ_Message *msg) {
  ...
  if (status != AJ_OK) {
    AJ_ErrPrintf(
        ("EncryptMesssage(): peer %s not authenticated", msg->destination));
    AJ_ErrPrintf(("EncryptMessage(): AJ_ERR_SECURITY\n"));
    status = AJ_ERR_SECURITY;
  } else {
    InitNonce(msg, role, nonce);
    status = AJ_Encrypt_CCM(key, ioBuf->bufStart, mlen, hlen, MAC_LENGTH, nonce,
                            sizeof(nonce));
  }
  return status;
}
...
AJ_Status AJ_DeliverMsg(AJ_Message *msg) {
  ...
  /*
   * If the header has already been marshaled (due to partial delivery) it will
   * be NULL
   */
  if (msg->hdr) {
    /*
...
static AJ_Status LoadBytes(AJ_IOBuffer *ioBuf, uint16_t numBytes, uint8_t pad) {
  ...
  while (AJ_IO_BUF_AVAIL(ioBuf) < numBytes) {
    ...
    status = ioBuf->x1.recv(ioBuf, numBytes - AJ_IO_BUF_AVAIL(ioBuf),
                            UNMARSHAL_TIMEOUT);
    if (status != AJ_OK) {
      /*
 ...
 */
static AJ_Status WriteBytes(AJ_Message *msg, const void *data, size_t numBytes,
                            size_t pad) {
  AJ_Status status = AJ_OK;
  AJ_IOBuffer *ioBuf = &msg->bus->sock.tx;
  if (numBytes && !data) {
  ...
  while (numBytes + pad) {
    ...
    if ((numBytes + pad) > canWrite) {
      /*
       * If we have already marshaled the header we can write what we have in
       * the buffer
       */
      if (msg->hdr) {
 ...
 */
static AJ_Status UnmarshalStruct(AJ_Message *msg, const char **sig, AJ_Arg *arg,
                                 uint8_t pad) {
  AJ_IOBuffer *ioBuf = &msg->bus->sock.rx;
...
/*
 * Indicates that scalar arrays should be unmarshaled element by element. This
 * value is set to TRUE
 * in AJ_UnmarshalContainer() for the duration of a single call to
 * AJ_UnmarshalArgs(). If we ever
 * have to make the unmarshaler thread-safe this will need to be moved into
 * AJ_Message.
 */
 ...
 */
static AJ_Status UnmarshalArray(AJ_Message *msg, const char **sig, AJ_Arg *arg,
                                uint8_t pad) {
  AJ_Status status;
  AJ_IOBuffer *ioBuf = &msg->bus->sock.rx;
  char typeId = **sig;
  ...
  ioBuf->readPtr += 4;
  /*
   * We are already aligned on 4 byte boundary but there may be padding after
   * the array length if
  ...
  if (!unmarshalScalarAsElement && IsScalarType(typeId)) {
    /*
     * For scalar types we do an inplace endian swap (if needed) and return a
     * pointer into the read buffer.
     */
...
static AJ_Status Unmarshal(AJ_Message *msg, const char **sig, AJ_Arg *arg) {
  ...
  *sig += 1;
  ...
  if (IsScalarType(typeId)) {
    ...
    /*
     * Read the string length. Note the length doesn't include the terminating
     * NUL
...
static const AJ_MsgHeader internalErrorHdr = {
    HOST_ENDIANESS, AJ_MSG_ERROR, 0, 0, 0, 1, 0};
...
static AJ_Status ValidateHeader(const AJ_Message *msg) {
   ...
   */
  if ((msg->sender) && (msg->x1.objPath)) {
    /*
     * Check required fields are present for each message type
     */
    switch (msg->hdr->msgType) {
    case AJ_MSG_SIGNAL:
      if (msg->iface && msg->x2.member) {
        status = AJ_OK;
      }
      break;

    case AJ_MSG_METHOD_CALL:
      if (msg->destination && msg->iface && msg->x2.member) {
        status = AJ_OK;
      }
      break;

    case AJ_MSG_ERROR:
      if (msg->destination && msg->x2.error && msg->x1.replySerial) {
        status = AJ_OK;
      } else {
    ...
    case AJ_MSG_METHOD_RET:
      if (msg->destination && msg->x1.replySerial) {
        status = AJ_OK;
      }
      break;
    }
  ...
  if (status != AJ_OK) {
    AJ_ErrPrintf(
        ("Header does not contain the required fields for the message type\n"));
  }
  return status;
}
...
AJ_Status AJ_ResetArgs(AJ_Message *msg) {
  ...
  if (status == AJ_OK) {
    AJ_IOBuffer *ioBuf = &msg->bus->sock.rx;
    size_t hdrSize = sizeof(AJ_MsgHeader) + msg->hdr->headerLen +
                     ((8 - msg->hdr->headerLen) & 7);
    /*
     * Args have already been converted to native endianess in place in the
     * input buffer, this
...
AJ_Status AJ_UnmarshalMsg(AJ_BusAttachment *bus, AJ_Message *msg,
                          uint32_t timeout) {
  AJ_Status status;
   ...
   */
  if ((ioBuf->readPtr < ioBuf->bufStart) ||
      (ioBuf->readPtr > (ioBuf->bufStart + ioBuf->bufSize))) {
    AJ_ErrPrintf(
        ("AJ_UnmarshalMsg(): Read pointer out of bounds: AJ_ERR_IO_BUFFER\n"));
  ...
  while (AJ_IO_BUF_AVAIL(ioBuf) < sizeof(AJ_MsgHeader)) {
    //#pragma calls = AJ_Net_Recv
    status = ioBuf->x1.recv(
        ioBuf, sizeof(AJ_MsgHeader) - AJ_IO_BUF_AVAIL(ioBuf), timeout);
    if (status != AJ_OK) {
      if (status == AJ_ERR_TIMEOUT) {
        /*
      ...
      /*
       * If there were no messages to receive check if we have any methods call
       * that have
       * timed-out and if so generate an internal error message to allow the
       * application to
       * proceed.
       */
      if ((status == AJ_ERR_TIMEOUT) && AJ_TimedOutMethodCall(msg)) {
        msg->hdr = (AJ_MsgHeader *)&internalErrorHdr;
        msg->x2.error = AJ_ErrTimeout;
   ...
   */
  if ((msg->hdr->endianess != AJ_LITTLE_ENDIAN) &&
      (msg->hdr->endianess != AJ_BIG_ENDIAN)) {
  ...
  if (msg->hdr->headerLen > (ioBuf->bufSize - sizeof(AJ_MsgHeader))) {
    AJ_ErrPrintf(
        ("AJ_UnmarshalMsg(): Header was too large: AJ_ERR_HDR_CORRUPT\n"));
...
#endif
  /*
   * If the message is encrypted and the endianess of the message is different
   * than the local host
   * endianness we need to copy the header bytes before we unmarshal the header
   * and swizzle all
   * the integers.
   */
  if ((msg->hdr->endianess != HOST_ENDIANESS) &&
  ...
  while (ioBuf->readPtr < endOfHeader) {
    ...
    AJ_Arg hdrVal;
    /*
     * Custom unmarshal the header field - signature is "(yv)" so starts off
     * with STRUCT aligment.
     */
     ...
     */
    if ((fieldId <= AJ_HDR_SESSION_ID) &&
        (TypeForHdr[fieldId] != hdrVal.typeId)) {
    ...
    case AJ_HDR_OBJ_PATH:
      msg->x1.objPath = hdrVal.val.v_objPath;
      break;
    ...
    case AJ_HDR_MEMBER:
      msg->x2.member = hdrVal.val.v_string;
      break;

    case AJ_HDR_ERROR_NAME:
      msg->x2.error = hdrVal.val.v_string;
      break;

    case AJ_HDR_REPLY_SERIAL:
      msg->x1.replySerial = *(hdrVal.val.v_uint32);
      break;
  ...
  /*
   * If we copied the raw header earlier we copy it back now. This only happens
   * if the message is
   * encrypted and the endianness of the message is different from the local
   * host's endianness.
   */
  if (hdrRaw) {
  ...
  if (status == AJ_OK) {
     ...
     *
     * Note we must do this after decrypting the message or message
...
AJ_Status AJ_SkipArg(AJ_Message *msg) {
  ...
  if (TYPE_FLAG(skippy.typeId) & AJ_CONTAINER) {
    ...
    if (skippy.typeId == AJ_ARG_ARRAY) {
       ...
       */
      char close = (skippy.typeId == AJ_ARG_STRUCT) ? AJ_STRUCT_CLOSE
                                                    : AJ_DICT_ENTRY_CLOSE;
...
AJ_Status AJ_UnmarshalArg(AJ_Message *msg, AJ_Arg *arg) {
  ...
  if (msg->varOffset) {
    ...
    if (container->typeId == AJ_ARG_ARRAY) {
      size_t len =
          (uint16_t)(ioBuf->readPtr - (uint8_t *)container->val.v_data);
      /*
...
static AJ_Status VUnmarshalArgs(AJ_Message *msg, const char *sig,
                                va_list *argpp) {
  AJ_Status status = AJ_OK;
  AJ_Arg arg;
  AJ_Arg container;
...
AJ_Status AJ_UnmarshalRaw(AJ_Message *msg, const void **data, size_t len,
                          size_t *actual) {
  AJ_Status status;
  size_t sz;
  AJ_IOBuffer *ioBuf = &msg->bus->sock.rx;
...
AJ_Status AJ_UnmarshalCloseContainer(AJ_Message *msg, AJ_Arg *arg) {
  ...
  if (arg->typeId == AJ_ARG_ARRAY) {
    ...
    if ((arg->typeId == AJ_ARG_DICT_ENTRY) &&
        (*arg->sigPtr != AJ_DICT_ENTRY_CLOSE)) {
      AJ_ErrPrintf(("AJ_UnmarshalCloseContainer(): AJ_ERR_SIGNATURE\n"));
      return AJ_ERR_SIGNATURE;
    }
  }
...
static AJ_Status MarshalContainer(AJ_Message *msg, const char **sig,
                                  AJ_Arg *arg, uint8_t pad) {
  AJ_Status status;
  AJ_IOBuffer *ioBuf = &msg->bus->sock.tx;
...
static AJ_Status MarshalMsg(AJ_Message *msg, uint8_t msgType, uint32_t msgId,
                            uint8_t flags) {
  AJ_Status status = AJ_OK;
  AJ_IOBuffer *ioBuf = &msg->bus->sock.tx;
  uint8_t fieldId;
  ...
  /*
   * Use the msgId to lookup information in the object and interface
   * descriptions to
  ...
  /*
   * The wire-protocol calls this flag NO_AUTO_START we toggle the meaning in
   * the API
  ...
  for (fieldId = AJ_HDR_OBJ_PATH; fieldId <= AJ_HDR_SESSION_ID; ++fieldId) {
    ...
    case AJ_HDR_OBJ_PATH:
      if ((msgType == AJ_MSG_METHOD_CALL) || (msgType == AJ_MSG_SIGNAL)) {
        hdrVal.val.v_objPath = msg->x1.objPath;
      }
      break;
    ...
    case AJ_HDR_MEMBER:
      if (msgType != AJ_MSG_ERROR) {
        int32_t len = AJ_StringFindFirstOf(msg->x2.member, " ");
        hdrVal.val.v_string = msg->x2.member;
    ...
    case AJ_HDR_ERROR_NAME:
      if (msgType == AJ_MSG_ERROR) {
        hdrVal.val.v_string = msg->x2.error;
      }
      break;

    case AJ_HDR_REPLY_SERIAL:
      if ((msgType == AJ_MSG_METHOD_RET) || (msgType == AJ_MSG_ERROR)) {
        hdrVal.val.v_uint32 = &msg->x1.replySerial;
      }
      break;
    ...
    /*
     * Custom marshal the header field - signature is "(yv)" so starts off with
     * STRUCT aligment.
     */
    buf[0] = fieldId;
    buf[1] = 1;
  ...
  if (status == AJ_OK) {
     ...
     */
    msg->hdr->headerLen =
        (uint32_t)((ioBuf->writePtr - ioBuf->bufStart) - sizeof(AJ_MsgHeader));
    /*
...
AJ_Arg *AJ_InitArg(AJ_Arg *arg, uint8_t typeId, uint8_t flags, const void *val,
                   size_t len) {
  if (!IsBasicType(typeId)) {
    memset(arg, 0, sizeof(AJ_Arg));
    return NULL;
...
static AJ_Status VMarshalArgs(AJ_Message *msg, const char *sig,
                              va_list *argpp) {
  AJ_Status status = AJ_OK;
  AJ_Arg arg;
  AJ_Arg container;
...
AJ_Status AJ_DeliverMsgPartial(AJ_Message *msg, uint32_t bytesRemaining) {
  ...
  /*
   * Partial delivery not currently supported for messages that must be
   * encrypted.
   */
  if (msg->hdr->flags & AJ_FLAG_ENCRYPTED) {
...
AJ_Status AJ_MarshalCloseContainer(AJ_Message *msg, AJ_Arg *arg) {
  ...
  if (arg->typeId == AJ_ARG_ARRAY) {
    uint32_t lenOffset =
        (uint32_t)((uint8_t *)arg->val.v_data - ioBuf->bufStart);
    /*
     * The length we marshal does not include the length field itself.
     */
    arg->len = (uint16_t)(ioBuf->writePtr - (uint8_t *)arg->val.v_data) - 4;
    /*
     * If the array element is 8 byte aligned and the array is not empty check
     * if there was
     * padding after the length. The length we marshal should not include the
     * padding.
     */
...
AJ_Status AJ_MarshalMethodCall(AJ_BusAttachment *bus, AJ_Message *msg,
                               uint32_t msgId, const char *destination,
                               AJ_SessionId sessionId, uint8_t flags,
                               uint32_t timeout) {
  AJ_Status status;
...
AJ_Status AJ_MarshalSignal(AJ_BusAttachment *bus, AJ_Message *msg,
                           uint32_t msgId, const char *destination,
                           AJ_SessionId sessionId, uint8_t flags,
                           uint32_t ttl) {
  memset(msg, 0, sizeof(AJ_Message));
...
AJ_Status AJ_MarshalReplyMsg(const AJ_Message *methodCall, AJ_Message *reply) {
  ...
  reply->x1.replySerial = methodCall->hdr->serialNum;
  return MarshalMsg(reply, AJ_MSG_METHOD_RET, methodCall->msgId,
                    methodCall->hdr->flags & AJ_FLAG_ENCRYPTED);
}

AJ_Status AJ_MarshalErrorMsg(const AJ_Message *methodCall, AJ_Message *reply,
                             const char *error) {
  AJ_ASSERT(methodCall->hdr->msgType == AJ_MSG_METHOD_CALL);
  memset(reply, 0, sizeof(AJ_Message));
  reply->bus = methodCall->bus;
  ...
  reply->sessionId = methodCall->sessionId;
  reply->x1.replySerial = methodCall->hdr->serialNum;
  reply->x2.error = error;
  return MarshalMsg(reply, AJ_MSG_ERROR, methodCall->msgId,
                    methodCall->hdr->flags & AJ_FLAG_ENCRYPTED);
}

AJ_Status AJ_MarshalStatusMsg(const AJ_Message *methodCall, AJ_Message *reply,
                              AJ_Status status) {
  switch (status) {
  ...
  case AJ_ERR_SECURITY:
    ...
    /*
     * We get a security violation error so if we encrypt the error message the
     * receiver
     * won't be able to decrypt it. We can fix this by clearing the header
     * flags.
     */
    if (status == AJ_OK) {
      reply->hdr->flags = 0;
