 ...
 */
static const AJ_Object *objectLists[AJ_MAX_OBJECT_LISTS] = {AJ_StandardObjects,
                                                            NULL, NULL};
 ...
 */
#define AJ_OBJ_FLAGS_INTROSPECTABLE_EXCLUDE_MASK                               \
  (AJ_OBJ_FLAG_HIDDEN | AJ_OBJ_FLAG_DISABLED | AJ_OBJ_FLAG_IS_PROXY)
 ...
 */
static AJ_DescriptionLookupFunc descriptionLookups[AJ_MAX_OBJECT_LISTS] = {
    NULL, NULL, NULL};
...
#define SESSIONLESS                                                            \
  '&' /* Only to be uesd with a signal member types, indicates that the signal \
         \                                                                     \
         \ \                                                                             \
         will not require a session and is a sessionless signal */
#define IS_SESSIONLESS(c) ((c) == SESSIONLESS)

#define SECURE_TRUE                                                            \
  '$' /* Security is required for an interface that start with a '$' character \
       * \                                                                     \
       * \ \                                                                             \
         */
#define SECURE_OFF                                                             \
  '#' /* Security is OFF, i.e. never required for an interface that starts \ \                                                                             \
         with a '#' character */

static const char *const MemberOpen[] = {"  <signal", "  <method",
                                         "  <property"};

static const char *const MemberClose[] = {
    "  </signal>\n", "  </method>\n", "/>\n",
    "  </property>\n", /* One element larger than the close to allow for a
                          property to encompass a description tag */
};

static const char *const Access[] = {
    "\" access=\"write\"", "\" access=\"readwrite\"", "\" access=\"read\""};

/*
 * Table contains 6 elements to allow for the ability to encompass a description
 * tag if one exists.
 * The computed index for closure of the tag has 3 added to it when a
...
static const char annotateSecure[] =
    "  <annotation name=\"org.alljoyn.Bus.Secure\" value=\"";
static const char secureTrue[] = "true\"/>\n";
static const char secureOff[] = "off\"/>\n";

static char ExpandAttribute(XMLWriterFunc XMLWriter, void *context,
                            const char **str, const char *pre,
                            const char *post) {
  uint32_t len = 0;
  char next = 0;
  const char *s = *str;
...
static void XMLWriteTag(XMLWriterFunc XMLWriter, void *context, const char *tag,
                        const char *attr, const char *val, uint32_t valLen,
                        uint8_t atom) {
...
static const char *GetDescription(AJ_DescriptionLookupFunc descLookup,
                                  uint32_t descId, const char *languageTag) {
  if (descLookup == NULL) {
    return NULL;
  }
...
static void XMLWriteDescription(XMLWriterFunc XMLWriter, void *context,
                                uint8_t level, const char *description,
...
static AJ_Status ExpandInterfaces(XMLWriterFunc XMLWriter, void *context,
                                  const AJ_InterfaceDescription *iface,
                                  AJ_DescriptionLookupFunc descLookup,
                                  uint32_t descObjId, const char *languageTag) {
  uint32_t descId;
  ...
  while (*iface) {
    ...
    if ((flag == SECURE_TRUE) || (flag == SECURE_OFF)) {
      ...
      /*
       * If flagged as secure or not secure, skip the first char (the '$' or
       * '#') of the name
       */
      XMLWriteTag(XMLWriter, context, interfaceOpen, nameAttr, entries[0] + 1,
                  0, FALSE);
      if (!dBus_std_iface) {
      ...
      XMLWriteTag(XMLWriter, context, interfaceOpen, nameAttr, entries[0], 0,
                  FALSE);
    }
    ...
    while (*(++entries)) {
      ...
      if (memberType > 2) {
        AJ_ErrPrintf(
            ("ExpandInterfaces(): %s", AJ_StatusText(AJ_ERR_UNEXPECTED)));
      ...
      if (memberType == SIGNAL && IS_SESSIONLESS(*member)) {
        ...
        if (descLookup != NULL) {
          /*
           * If we have a descLookup function pointer it implies that we entered
           * the method
      ...
      if (memberType == PROPERTY) {
        ...
        while (attr) {
          ...
          if (IS_DIRECTION(*member)) {
            ...
            dir = ExpandAttribute(XMLWriter, context, &member, nameAttr, "\"") -
                  IN_ARG;
          }
          if ((dir != 0) && (dir != 2)) {
            AJ_ErrPrintf(
          ...
          /*
           * If we have a description then wait to close the tag until after
           * adding in a description tag
           */
          description = GetDescription(
              descLookup,
              (descId | (((uint32_t)memberIndex) << 8) | ((uint32_t)argIndex)),
              languageTag);
          if (description != NULL) {
            dir += 3;
          }
          attr = ExpandAttribute(XMLWriter, context, &member, typeAttr,
                                 Direction[dir]);
          if (description != NULL) {
            XMLWriteDescription(XMLWriter, context, 2, description,
                                languageTag);
      ...
      description = GetDescription(
          descLookup, (descId | (((uint32_t)memberIndex) << 8)), languageTag);
      if (description != NULL) {
...
/*
 * Check if the path c is child of path p if so return pointer to the start of
 * the child path
...
static const AJ_Object *FirstInstance(const char *path, const char *child,
                                      uint32_t sz) {
  AJ_ObjectIterator iter;
  const AJ_Object *obj =
      AJ_InitObjectIterator(&iter, AJ_OBJ_FLAGS_ALL_INCLUDE_MASK,
                            AJ_OBJ_FLAGS_INTROSPECTABLE_EXCLUDE_MASK);
...
/*
 * Security applies if the interface is secure or if the object or it's parent
 * object is flagged as
...
static uint32_t SecurityApplies(const char *ifc, const AJ_Object *obj) {
   ...
   */
  lookup = AJ_InitObjectIterator(&iter, AJ_OBJ_FLAGS_ALL_INCLUDE_MASK,
                                 AJ_OBJ_FLAG_IS_PROXY);
  while (lookup != NULL) {
    if ((lookup->flags & AJ_OBJ_FLAG_SECURE) &&
        ChildPath(lookup->path, obj->path, NULL)) {
      return TRUE;
    }
...
static AJ_Status GenXML(XMLWriterFunc XMLWriter, void *context,
                        const AJ_ObjectIterator *objIter,
                        const AJ_Object *virtualObject,
                        const char *languageTag) {
  ...
  if (obj != NULL && obj->path != NULL) {
    ...
    const AJ_Object *childObj =
        AJ_InitObjectIterator(&childObjectIter, AJ_OBJ_FLAGS_ALL_INCLUDE_MASK,
                              AJ_OBJ_FLAGS_INTROSPECTABLE_EXCLUDE_MASK);
    const char *description = NULL;
    ...
    if (objIter != NULL) {
      description =
          GetDescription(descLookup, (objIter->n - 1) << 24, languageTag);
    }
    if (description != NULL) {
      XMLWriteDescription(XMLWriter, context, 0, description, languageTag);
    }
    if (status == AJ_OK) {
      if (objIter != NULL) {
        status =
            ExpandInterfaces(XMLWriter, context, obj->interfaces, descLookup,
                             (objIter->n - 1) << 24, languageTag);
      } else {
        status = ExpandInterfaces(XMLWriter, context, obj->interfaces,
                                  descLookup, 0, languageTag);
      }
    }
    if (status == AJ_OK) {
      while (childObj != NULL) {
        ...
        /*
         * If there is a child check that this is the first instance of this
         * child.
         */
        if (child && (FirstInstance(obj->path, child, len) == childObj)) {
          ...
          description = GetDescription(
              descLookup, (childObjectIter.n - 1) << 24, languageTag);
          if (description != NULL) {
            XMLWriteTag(XMLWriter, context, nodeOpen, nameAttr, child, len,
                        FALSE);
            XMLWriteDescription(XMLWriter, context, 0, description,
                                languageTag);
    ...
    if (status != AJ_OK) {
      AJ_ErrPrintf(("\nFailed to generate XML - check interface descriptions "
                    "of %s for errors\n",
                    obj->path));
    }
  }
...
void AJ_PrintXMLWithDescriptions(const AJ_Object *objs,
  ...
  while (objs && objs->path) {
    if (strcmp(objs->path, "/") == 0) {
      status = GenXML(PrintXML, NULL, NULL, objs,
                      languageTag); // with descriptions in the given language
      if (status != AJ_OK) {
        AJ_ErrPrintf(("\nFailed to generate XML - check interface descriptions "
                      "of %s for errors\n",
                      objs->path));
      ...
      if (lookup != NULL) {
        status = GenXML(PrintXML, NULL, &iter, NULL,
                        languageTag); // with descriptions in the given language
        if (status != AJ_OK) {
          AJ_ErrPrintf(("\nFailed to generate XML - check interface "
                        "descriptions of %s for errors\n",
                        objs->path));
        }
      } else {
        AJ_AlwaysPrintf(("Reminder: Object not yet added to the ObjectList, do "
                         "not forget to call RegisterObjects\n"));
...
AJ_Status AJ_HandleIntrospectRequest(const AJ_Message *msg, AJ_Message *reply,
  ...
  const AJ_Object *obj =
      AJ_InitObjectIterator(&objIter, AJ_OBJ_FLAGS_ALL_INCLUDE_MASK,
                            AJ_OBJ_FLAGS_INTROSPECTABLE_EXCLUDE_MASK);
  const AJ_Object virtualObject = {msg->x1.objPath, NULL, 0, NULL};
  ...
  while (obj != NULL) {
     ...
     *
     * - The request has a complete object path to one of the application
     * objects.
     * - The request has a path to a parent object of one or more application
     * objects where the
     *   parent itself is just a place-holder in the object hierarchy.
     */
    if (strcmp(msg->x1.objPath, obj->path) == 0) {
      break;
    }
    if (ChildPath(msg->x1.objPath, obj->path, NULL)) {
      ++children;
    }
    /*
     * If there was not a direct match but the requested node has children we
     * create
  ...
  if (obj != NULL && obj->path != NULL) {
    ...
    if (status != AJ_OK) {
      AJ_ErrPrintf(
          ("AJ_HandleIntrospectRequest(): Failed to generate XML. status=%s",
     ...
     */
    AJ_InfoPrintf(
        ("AJ_HandleIntrospectRequest() %d bytes of XML\n", context.len));
     ...
     */
    AJ_WarnPrintf(
        ("AJ_HandleIntrospectRequest() NO MATCH for %s\n", msg->x1.objPath));
...
/*
 * Check that the signature in the message matches the encoded signature in the
 * string
 */
 ...
 */
static AJ_Status ComposeSignature(const char *encoding, char direction,
                                  char *sig, size_t len) {
  while (*encoding) {
    /*
     * Skip until we find a direction character
...
static AJ_Status MatchProp(const char *member, const char *prop, uint8_t op,
                           const char **sig) {
...
static uint32_t MatchMember(const char *encoding, const AJ_Message *msg) {
  const char *member = msg->x2.member;
  uint8_t memberType =
      (msg->hdr->msgType == AJ_MSG_METHOD_CALL) ? METHOD : SIGNAL;
...
static AJ_InterfaceDescription
FindInterface(const AJ_InterfaceDescription *interfaces, const char *iface,
              uint8_t *index) {
  *index = 0;
  if (interfaces) {
    while (*interfaces) {
...
/*
 * Match the object path. There are two wild card entries for object paths: '?'
 * matches any method
 * call and '!' matches any signal.  The method call wildcard is specifically to
 * support the
 * introspection and ping methods. The signal wildcard allows for a single
 * handler for a specific
 * signal emitted by any object. Note that these wildcards are expected to
 * provide unique matches,
 * i.e. there should be no non-wildcarded entry in any object table that would
 * also match.
 */
static uint8_t inline MatchPath(const char *path, AJ_Message *msg) {
  ...
  return strcmp(path, msg->x1.objPath) == 0;
}
    ...
    for (; obj->path; ++pIndex, ++obj) {
      ...
      if (MatchPath(obj->path, msg)) {
        ...
        AJ_InterfaceDescription desc =
            FindInterface(obj->interfaces, msg->iface, &iIndex);
        if (desc) {
          uint8_t mIndex = 0;
          *secure = SecurityApplies(*desc, obj);
          /*
           * Skip the interface name and iterate over the members of the
           * interface
           */
          while (*(++desc)) {
            if (MatchMember(*desc, msg)) {
              msg->msgId =
                  (oIndex << 24) | (pIndex << 16) | (iIndex << 8) | mIndex;
...
static AJ_Status UnpackMsgId(uint32_t msgId, const char **objPath,
...
#ifndef NDEBUG
  if ((oIndex >= ArraySize(objectLists)) ||
      !CheckIndex(objectLists[oIndex], pIndex, sizeof(AJ_Object))) {
  ...
  *secure = SecurityApplies(*ifc, obj);
  ...
  if (member) {
     ...
     */
    if (MEMBER_TYPE(ifc[mIndex][0]) == SIGNAL &&
        IS_SESSIONLESS(ifc[mIndex][1])) {
      *member = ifc[mIndex] + 2;
    } else {
      *member = ifc[mIndex] + 1;
    }
...
AJ_Status AJ_MarshalPropertyArgs(AJ_Message *msg, uint32_t propId) {
  ...
  if (status != AJ_OK) {
    AJ_ErrPrintf(
        ("AJ_MarhsalPropertyArgs(): status=%s\n", AJ_StatusText(status)));
    return status;
  }
  if (secure) {
...
AJ_Status AJ_InitMessageFromMsgId(AJ_Message *msg, uint32_t msgId,
  ...
  /*
   * Static buffer for holding the signature for the message currently being
   * marshaled. Since this
   * implementation can only marshal one message at a time we only need one of
   * these buffers. The
   * size of the buffer dictates the maximum size signature we can marshal. The
   * wire protocol
   * allows up to 255 characters in a signature but that would represent an
   * outgrageously complex
  ...
  if (msgType == AJ_MSG_ERROR) {
    ...
    /*
     * Get the secure flag if we have a valid message id this will ensure that
     * the error
    ...
    if (msgType == AJ_MSG_METHOD_RET) {
      ...
      msg->msgId = msgId;
      status =
          UnpackMsgId(msgId, &msg->x1.objPath, &msg->iface, &member, secure);
      if (status == AJ_OK) {
        /*
        ...
        } else if (*msg->x1.objPath == '*') {
          /*
           * The wildcard object path is a special for case methods implemented
           * by all
           * objects. In the message header need a valid object path, it doesn't
           * matter
           * which one by definition so use the root object because it is always
           * valid.
           */
          msg->x1.objPath = "/";
        } else if (*msg->x1.objPath != '/') {
     ...
     */
    if (status == AJ_OK) {
      status = ComposeSignature(member, direction, msgSignature,
                                sizeof(msgSignature));
      if (status == AJ_OK) {
...
AJ_Status AJ_IdentifyProperty(AJ_Message *msg, const char *iface,
                              ...
                              const char **sigPtr, uint8_t *secure) {
  AJ_Status status = AJ_OK;
  ...
  *propId = AJ_INVALID_PROP_ID;
  ...
  if (desc) {
    ...
    *secure = SecurityApplies(*desc, obj);
    ...
    /*
     * Iterate over the interface members to locate the property that is being
     * accessed.
     */
    while (*(++desc)) {
...
AJ_Status AJ_IdentifyMessage(AJ_Message *msg) {
  ...
  if ((msg->hdr->msgType == AJ_MSG_METHOD_CALL) ||
    ...
    /*
     * Generate an error response for an invalid method call rather than
     * reporting an invalid
    ...
    ReplyContext *repCtx = FindReplyContext(msg->x1.replySerial);
    if (repCtx) {
...
AJ_Status AJ_AllocReplyContext(AJ_Message *msg, uint32_t timeout) {
  if (msg->hdr->flags & AJ_FLAG_NO_REPLY_EXPECTED) {
    ...
    if (repCtx) {
      ...
      AJ_ErrPrintf(("AJ_AllocReplyContext(): Failed to allocate reply context. "
...
uint8_t AJ_TimedOutMethodCall(AJ_Message *msg) {
  ...
  for (i = 0; i < ArraySize(replyContexts); ++i, ++repCtx) {
    if (repCtx->serial &&
        (AJ_GetElapsedTime(&repCtx->callTime, TRUE) > repCtx->timeout)) {
      /*
       * Set the reply serial and message id for the timeout error
       */
      msg->x1.replySerial = repCtx->serial;
...
AJ_Status AJ_SetObjectFlags(const char *objPath, uint8_t setFlags,
  ...
  if (list && objPath) {
     ...
     */
    while (list->path) {
      if ((strcmp(objPath, list->path) == 0) ||
          ChildPath(objPath, list->path, NULL)) {
        list->flags &= ~clearFlags;
        list->flags |= setFlags;
        AJ_InfoPrintf(
            ("Setting flags for %s to 0x%02x\n", list->path, list->flags));
        status = AJ_OK;
      }
      ++list;
    }
  ...
  while (iter->l < ArraySize(objectLists)) {
    ...
    if (list) {
      while (list[iter->n].path) {
        ...
        /*
         * For backwards compatibility the third entry is reserved for proxy
         * objects. Going forward
         * proxy objects are identified in the object list by the
         * AJ_OBJ_FLAG_IS_PROXY.
         */
        if (iter->l == AJ_PRX_ID_FLAG) {
          objFlags &= AJ_OBJ_FLAG_IS_PROXY;
        }
        if ((objFlags & iter->fin ||
             (objFlags == 0 && iter->fin == AJ_OBJ_FLAGS_ALL_INCLUDE_MASK)) &&
            !(objFlags & iter->fex)) {
          return obj;
        }
      }
