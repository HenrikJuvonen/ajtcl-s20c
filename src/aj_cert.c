...
AJ_Status AJ_BigEndianEncodePrivateKey(ecc_privatekey *privatekey,
                                       uint8_t *b8) {
  HostU32ToBigEndianU8((uint32_t *)privatekey, sizeof(ecc_privatekey), b8);
  return AJ_OK;
}

AJ_Status AJ_BigEndianDecodePrivateKey(ecc_privatekey *privatekey,
                                       uint8_t *b8) {
...
static void CertificateSize(AJ_Certificate *certificate) {
  certificate->size =
      sizeof(uint32_t) + sizeof(ecc_publickey) + SHA256_DIGEST_LENGTH;
  switch (certificate->version) {
  case 1:
  case 2:
    certificate->size +=
        sizeof(ecc_publickey) + 2 * sizeof(uint64_t) + sizeof(uint8_t);
    break;
  }
  switch (certificate->version) {
  case 2:
...
AJ_Status AJ_BigEndianEncodeCertificate(AJ_Certificate *certificate,
  ...
  case 2:
    ...
    b8 += sizeof(ecc_publickey);
    HostU64ToBigEndianU8(&certificate->validity.validfrom, sizeof(uint64_t),
                         b8);
    b8 += sizeof(uint64_t);
  ...
  HostU32ToBigEndianU8((uint32_t *)&certificate->signature,
                       sizeof(ecc_signature), b8);
...
AJ_Status AJ_BigEndianDecodeCertificate(AJ_Certificate *certificate,
  ...
  case 2:
    ...
    b8 += sizeof(ecc_publickey);
    BigEndianU8ToHostU64(b8, &certificate->validity.validfrom,
                         sizeof(uint64_t));
    b8 += sizeof(uint64_t);
  ...
  b8 += SHA256_DIGEST_LENGTH;
  BigEndianU8ToHostU32(b8, (uint32_t *)&certificate->signature,
...
AJ_Status AJ_CreateCertificate(AJ_Certificate *certificate,
  ...
  memcpy((uint8_t *)&certificate->issuer, (uint8_t *)issuer,
         sizeof(ecc_publickey));
  switch (version) {
  case 1:
  case 2:
    memcpy((uint8_t *)&certificate->subject, (uint8_t *)subject,
           sizeof(ecc_publickey));
  ...
  memcpy((uint8_t *)&certificate->digest, (uint8_t *)digest,
...
static AJ_Status CertificateDigest(AJ_Certificate *certificate,
                                   uint8_t *digest) {
...
AJ_Status AJ_SignCertificate(AJ_Certificate *certificate,
  ...
  switch (certificate->version) {
  case 0:
    status = AJ_DSASignDigest(certificate->digest, issuer_private,
...
AJ_Status AJ_VerifyCertificate(AJ_Certificate *certificate) {
  ...
  switch (certificate->version) {
  case 0:
    status = AJ_DSAVerifyDigest(certificate->digest, &certificate->signature,
                                &certificate->issuer);
    break;
  ...
  case 2:
    status = CertificateDigest(certificate, digest);
    status = AJ_DSAVerifyDigest(digest, &certificate->signature,
                                &certificate->issuer);
    break;
  }
