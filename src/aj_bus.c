...
AJ_Status AJ_BusRequestName(AJ_BusAttachment *bus, const char *name,
  ...
  status = AJ_MarshalMethodCall(bus, &msg, AJ_METHOD_REQUEST_NAME,
                                AJ_BusDestination, 0, 0, AJ_METHOD_TIMEOUT);
  if (status == AJ_OK) {
...
AJ_Status AJ_BusReleaseName(AJ_BusAttachment *bus, const char *name) {
  ...
  status = AJ_MarshalMethodCall(bus, &msg, AJ_METHOD_RELEASE_NAME,
                                AJ_BusDestination, 0, 0, AJ_METHOD_TIMEOUT);
  if (status == AJ_OK) {
...
AJ_Status AJ_BusAdvertiseName(AJ_BusAttachment *bus, const char *name,
  ...
  AJ_Status status;
  AJ_Message msg;
  uint32_t msgId = (op == AJ_BUS_START_ADVERTISING)
                       ? AJ_METHOD_ADVERTISE_NAME
                       : AJ_METHOD_CANCEL_ADVERTISE;

  AJ_InfoPrintf(("AJ_BusAdvertiseName(bus=0x%p, name=\"%s\", "
                 "transportMask=0x%x, op=%d.)\n",
                 bus, name, transportMask, op));

  status = AJ_MarshalMethodCall(bus, &msg, msgId, AJ_BusDestination, 0, flags,
                                AJ_METHOD_TIMEOUT);
  if (status == AJ_OK) {
...
AJ_Status AJ_BusFindAdvertisedName(AJ_BusAttachment *bus,
  ...
  AJ_Status status;
  AJ_Message msg;
  uint32_t msgId = (op == AJ_BUS_START_FINDING) ? AJ_METHOD_FIND_NAME
                                                : AJ_METHOD_CANCEL_FIND_NAME;

  AJ_InfoPrintf(
      ("AJ_BusFindAdvertiseName(bus=0x%p, namePrefix=\"%s\", op=%d.)\n", bus,
       namePrefix, op));

  status = AJ_MarshalMethodCall(bus, &msg, msgId, AJ_BusDestination, 0, 0,
                                AJ_METHOD_TIMEOUT);
  if (status == AJ_OK) {
...
AJ_Status AJ_BusFindAdvertisedNameByTransport(AJ_BusAttachment *bus,
  ...
  AJ_Status status;
  AJ_Message msg;
  uint32_t msgId = (op == AJ_BUS_START_FINDING)
                       ? AJ_METHOD_FIND_NAME_BY_TRANSPORT
                       : AJ_METHOD_CANCEL_FIND_NAME_BY_TRANSPORT;

  AJ_InfoPrintf(("AJ_BusFindAdvertiseNameByTransport(bus=0x%p, "
                 "namePrefix=\"%s\", transport=%d., op=%d.)\n",
...
static AJ_Status MarshalSessionOpts(AJ_Message *msg,
 ...
 */
static const AJ_SessionOpts defaultSessionOpts = {AJ_SESSION_TRAFFIC_MESSAGES,
                                                  AJ_SESSION_PROXIMITY_ANY,
                                                  AJ_TRANSPORT_ANY, FALSE};
...
AJ_Status AJ_BusBindSessionPort(AJ_BusAttachment *bus, uint16_t port,
  ...
  status = AJ_MarshalMethodCall(bus, &msg, AJ_METHOD_BIND_SESSION_PORT,
                                AJ_BusDestination, 0, flags, AJ_METHOD_TIMEOUT);
  if (status == AJ_OK) {
...
AJ_Status AJ_BusUnbindSession(AJ_BusAttachment *bus, uint16_t port) {
  ...
  status = AJ_MarshalMethodCall(bus, &msg, AJ_METHOD_UNBIND_SESSION,
                                AJ_BusDestination, 0, 0, AJ_METHOD_TIMEOUT);
  if (status == AJ_OK) {
    AJ_MarshalArgs(&msg, "q", port);
  }
...
AJ_Status AJ_BusCancelSessionless(AJ_BusAttachment *bus, uint32_t serialNum) {
  ...
  AJ_InfoPrintf(
      ("AJ_BusCancelSessionless(bus=0x%p, serialNum=%d.)\n", bus, serialNum));

  status = AJ_MarshalMethodCall(bus, &msg, AJ_METHOD_CANCEL_SESSIONLESS,
                                AJ_BusDestination, 0, 0, AJ_METHOD_TIMEOUT);
  if (status == AJ_OK) {
...
AJ_Status AJ_BusJoinSession(AJ_BusAttachment *bus, const char *sessionHost,
  ...
  AJ_InfoPrintf(
      ("AJ_BusJoinSession(bus=0x%p, sessionHost=\"%s\", port=%d., opts=0x%p)\n",
       bus, sessionHost, port, opts));

  if (!opts) {
    opts = &defaultSessionOpts;
  }
  status = AJ_MarshalMethodCall(bus, &msg, AJ_METHOD_JOIN_SESSION,
                                AJ_BusDestination, 0, 0, AJ_METHOD_TIMEOUT);
  if (status == AJ_OK) {
...
AJ_Status AJ_BusLeaveSession(AJ_BusAttachment *bus, uint32_t sessionId) {
  ...
  AJ_InfoPrintf(
      ("AJ_BusLeaveSession(bus=0x%p, sessionId=%d.)\n", bus, sessionId));

  status = AJ_MarshalMethodCall(bus, &msg, AJ_METHOD_LEAVE_SESSION,
                                AJ_BusDestination, 0, 0, AJ_METHOD_TIMEOUT);
  if (status == AJ_OK) {
...
AJ_Status AJ_BusSetLinkTimeout(AJ_BusAttachment *bus, uint32_t sessionId,
  ...
  AJ_InfoPrintf(
      ("AJ_BusSetLinkTimeout(bus=0x%p, sessionId=%d., linkTimeout=%d.)\n", bus,
       sessionId, linkTimeout));

  status = AJ_MarshalMethodCall(bus, &msg, AJ_METHOD_SET_LINK_TIMEOUT,
                                AJ_BusDestination, 0, 0, AJ_METHOD_TIMEOUT);
  if (status == AJ_OK) {
...
AJ_Status AJ_BusSetSignalRule(AJ_BusAttachment *bus, const char *ruleString,
  ...
  AJ_Status status;
  AJ_Message msg;
  uint32_t msgId = (rule == AJ_BUS_SIGNAL_ALLOW) ? AJ_METHOD_ADD_MATCH
                                                 : AJ_METHOD_REMOVE_MATCH;

  AJ_InfoPrintf(("AJ_BusSetSignalRule(bus=0x%p, ruleString=\"%s\", rule=%d.)\n",
                 bus, ruleString, rule));

  status = AJ_MarshalMethodCall(bus, &msg, msgId, AJ_DBusDestination, 0, 0,
                                AJ_METHOD_TIMEOUT);
  if (status == AJ_OK) {
    uint32_t sz = 0;
    uint8_t nul = 0;
...
AJ_Status AJ_BusAddSignalRule(AJ_BusAttachment *bus, const char *signalName,
  ...
  AJ_InfoPrintf(("AJ_BusAddSignalRule(bus=0x%p, signalName=\"%s\", "
                 "interfaceName=\"%s\", rule=%d.)\n",
...
AJ_Status AJ_BusSetSignalRuleFlags(AJ_BusAttachment *bus,
  ...
  status = AJ_MarshalMethodCall(bus, &msg, msgId, AJ_DBusDestination, 0, flags,
                                AJ_METHOD_TIMEOUT);
  if (status == AJ_OK) {
    uint32_t sz = 0;
    uint8_t nul = 0;
...
AJ_Status AJ_BusReplyAcceptSession(AJ_Message *msg, uint32_t accept) {
  ...
  AJ_InfoPrintf(
      ("AJ_BusReplyAcceptSession(msg=0x%p, accept=%d.)\n", msg, accept));
...
AJ_Status AJ_BusRemoveSessionMember(AJ_BusAttachment *bus, uint32_t sessionId,
  ...
  AJ_InfoPrintf(
      ("AJ_BusRemoveSessionMember(bus=0x%p, sessionId=%d, member=%s.)\n", bus,
       sessionId, member));
  status = AJ_MarshalMethodCall(bus, &msg, AJ_METHOD_REMOVE_SESSION_MEMBER,
                                AJ_BusDestination, 0, 0, AJ_METHOD_TIMEOUT);
  if (status == AJ_OK) {
...
AJ_Status AJ_BusPing(AJ_BusAttachment *bus, const char *name,
                     uint32_t timeout) {
  AJ_Status status;
  AJ_Message msg;

  AJ_InfoPrintf(
      ("AJ_BusPing(bus=0x%p, name=%s, timeout=%d)\n", bus, name, timeout));
  status = AJ_MarshalMethodCall(bus, &msg, AJ_METHOD_BUS_PING,
                                AJ_BusDestination, 0, 0, AJ_METHOD_TIMEOUT);
  if (status == AJ_OK) {
...
AJ_Status AJ_BusHandleBusMessage(AJ_Message *msg) {
  ...
  case AJ_METHOD_EXCHANGE_GROUP_KEYS:
    AJ_InfoPrintf(
        ("AJ_BusHandleBusMessage(): AJ_METHOD_EXCHANGE_GROUP_KEYS\n"));
  ...
  case AJ_REPLY_ID(AJ_METHOD_EXCHANGE_GUIDS):
    AJ_InfoPrintf(
        ("AJ_BusHandleBusMessage(): AJ_REPLY_ID(AJ_METHOD_EXCHANGE_GUIDS)\n"));
    status = AJ_PeerHandleExchangeGUIDsReply(msg);
    break;

  case AJ_REPLY_ID(AJ_METHOD_EXCHANGE_SUITES):
    AJ_InfoPrintf(
        ("AJ_BusHandleBusMessage(): AJ_REPLY_ID(AJ_METHOD_EXCHANGE_SUITES)\n"));
    status = AJ_PeerHandleExchangeSuitesReply(msg);
    break;

  case AJ_REPLY_ID(AJ_METHOD_KEY_EXCHANGE):
    AJ_InfoPrintf(
        ("AJ_BusHandleBusMessage(): AJ_REPLY_ID(AJ_METHOD_KEY_EXCHANGE)\n"));
  ...
  case AJ_SIGNAL_NAME_ACQUIRED:
    AJ_InfoPrintf(("AJ_BusHandleBusMessage(): "
                   "AJ_SIGNAL_{SESSION_JOINED|NAME_ACQUIRED}\n"));
    // nothing to do here
    status = AJ_OK;
    break;
  ...
  case AJ_REPLY_ID(AJ_METHOD_ADVERTISE_NAME):
    AJ_InfoPrintf(("AJ_BusHandleBusMessage(): "
                   "AJ_REPLY_ID(AJ_METHOD_{ADD_MATCH|CANCEL_ADVERTISE|"
                   "ADVERTISE_NAME})\n"));
  ...
  case AJ_SIGNAL_NAME_OWNER_CHANGED:
    AJ_InfoPrintf(
        ("AJ_BusHandleBusMessage(): AJ_SIGNAL_NAME_OWNER_CHANGED)\n"));
...
static AJ_Status AddSuite(AJ_BusAttachment *bus, uint32_t suite) {
  if (!IsSuiteAvailable(bus, suite)) {
    bus->suites = (uint32_t *)AJ_Realloc(bus->suites, (bus->numsuites + 1) *
                                                          sizeof(uint32_t));
    if (!bus->suites) {
...
void AJ_BusSetAuthListenerCallback(AJ_BusAttachment *bus,
  ...
  AJ_InfoPrintf(
      ("AJ_BusSetAuthListenerCallback(bus=0x%p, authListenerCallback=0x%p)\n",
       bus, authListenerCallback));
  bus->authListenerCallback = authListenerCallback;
}

AJ_Status AJ_BusAuthenticatePeer(AJ_BusAttachment *bus, const char *peerName,
                                 AJ_BusAuthPeerCallback callback,
                                 void *cbContext) {
  AJ_InfoPrintf(("AJ_BusAuthenticatePeer(bus=0x%p, peerName=\"%s\", "
                 "callback=0x%p, cbContext=0x%p)\n",
...
static AJ_Status PropAccess(AJ_Message *msg, PropCallback *cb, uint8_t op) {
  ...
  AJ_InfoPrintf(("PropAccess(msg=0x%p, cb=0x%p, op=%s)\n", msg, cb,
                 (op == AJ_PROP_GET) ? "get" : "set"));

  /*
   * Find out which property is being accessed and whether the access is a GET
   * or SET
   */
  ...
  if (status == AJ_OK) {
    ...
    if (op == AJ_PROP_GET) {
      ...
      status = cb->x1.Get(&reply, propId, cb->context);
    } else {
      const char *variant;
      ...
      if (strcmp(sig, variant) == 0) {
        status = cb->x1.Set(msg, propId, cb->context);
      } else {
