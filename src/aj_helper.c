...
static Timer Timers[AJ_MAX_TIMERS] = {{NULL}};
...
AJ_Status AJ_RunAllJoynService(AJ_BusAttachment *bus,
  ...
  while (TRUE) {
    ...
    if (!connected) {
      status =
          AJ_StartService(bus, config->daemonName, config->connect_timeout,
                          config->connected, config->session_port,
                          config->service_name, config->flags, config->opts);
      if (status != AJ_OK) {
        AJ_ErrPrintf(
            ("AJ_RunAllJoynService(): status=%s.\n", AJ_StatusText(status)));
        continue;
      }
    ...
    if (status == AJ_OK) {
      ...
      // check the user's handlers first.  ANY message that AllJoyn can handle
      // is override-able.
      ...
      // these are stored in an array because multiple getters and setters can
      // exist if running more than one bus object
      while (handled != TRUE && prop_entry->msgid != 0) {
        if (prop_entry->msgid == msg.msgId) {
          ...
          if (method == AJ_PROP_GET) {
            status =
                AJ_BusPropGet(&msg, prop_entry->callback, prop_entry->context);
          } else if (method == AJ_PROP_SET) {
            status =
                AJ_BusPropSet(&msg, prop_entry->callback, prop_entry->context);
          } else {
...
AJ_Status AJ_StartService(AJ_BusAttachment *bus, const char *daemonName,
  ...
  AJ_InfoPrintf(
      ("AJ_StartService(bus=0x%p, daemonName=\"%s\", timeout=%d., "
       "connected=%d., port=%d., name=\"%s\", flags=0x%x, opts=0x%p)\n",
  ...
  while (TRUE) {
    ...
    if (!connected) {
      ...
      if (status != AJ_OK) {
        AJ_WarnPrintf(
            ("AJ_StartService(): connect failed: sleeping for %d seconds\n",
  ...
  while (!serviceStarted && (status == AJ_OK)) {
    ...
    case AJ_REPLY_ID(AJ_METHOD_REQUEST_NAME):
      if (msg.hdr->msgType == AJ_MSG_ERROR) {
        AJ_ErrPrintf(
            ("AJ_StartService(): AJ_METHOD_REQUEST_NAME: %s\n", msg.x2.error));
        status = AJ_ERR_FAILURE;
      } else {
        AJ_InfoPrintf(("AJ_StartService(): AJ_BusAdvertiseName()\n"));
        status = AJ_BusAdvertiseName(bus, name, AJ_TRANSPORT_ANY,
...
static AJ_Status StartClient(AJ_BusAttachment *bus, const char *daemonName,
                             uint32_t timeout, uint8_t connected,
                             const char *name, uint16_t port,
                             const char **interfaces, uint32_t *sessionId,
                             char *serviceName, const AJ_SessionOpts *opts) {
  AJ_Status status = AJ_OK;
  AJ_Time timer;
  uint8_t found = FALSE;
  ...
  AJ_InfoPrintf(("AJ_StartClient(bus=0x%p, daemonName=\"%s\", timeout=%d., "
                 "connected=%d., interface=\"%p\", sessionId=0x%p, "
                 "serviceName=0x%p, opts=0x%p)\n",
                 bus, daemonName, timeout, connected, interfaces, sessionId,
                 serviceName, opts));
  ...
  while (elapsed < timeout) {
    if (!connected) {
      ...
      if (status != AJ_OK) {
        ...
        AJ_WarnPrintf(("AJ_StartClient(): Failed to connect to bus, sleeping "
                       "for %d seconds\n",
                       AJ_CONNECT_PAUSE / 1000));
        AJ_Sleep(AJ_CONNECT_PAUSE);
        continue;
    ...
    if (!connected) {
      AJ_WarnPrintf(
          ("AJ_StartClient(): Client disconnecting from bus: status=%s.\n",
           AJ_StatusText(status)));
      AJ_Disconnect(bus);
    }
  }
  if (elapsed > timeout) {
    AJ_WarnPrintf(("AJ_StartClient(): Client timed-out trying to connect to "
                   "bus: status=%s.\n",
                   AJ_StatusText(status)));
    return AJ_ERR_TIMEOUT;
  }
  ...
  *sessionId = 0;
  ...
  while (!clientStarted && (status == AJ_OK)) {
    ...
    case AJ_REPLY_ID(AJ_METHOD_FIND_NAME_BY_TRANSPORT):
      if (msg.hdr->msgType == AJ_MSG_ERROR) {
        AJ_ErrPrintf(
            ("AJ_StartClient(): AJ_METHOD_FIND_NAME: %s\n", msg.x2.error));
        status = AJ_ERR_FAILURE;
      } else {
        uint32_t disposition;
        AJ_UnmarshalArgs(&msg, "u", &disposition);
        if ((disposition != AJ_FIND_NAME_STARTED) &&
            (disposition != AJ_FIND_NAME_ALREADY)) {
    ...
    case AJ_SIGNAL_ABOUT_ANNOUNCE: {
      ...
      AJ_InfoPrintf(
          ("AJ_StartClient(): AboutAnnounce from (%s)\n", msg.sender));
      if (!found) {
        found = TRUE;
    ...
    case AJ_REPLY_ID(AJ_METHOD_JOIN_SESSION): {
      ...
      if (msg.hdr->msgType == AJ_MSG_ERROR) {
        AJ_ErrPrintf(
            ("AJ_StartClient(): AJ_METHOD_JOIN_SESSION: %s\n", msg.x2.error));
        status = AJ_ERR_FAILURE;
      } else {
    ...
    case AJ_SIGNAL_SESSION_LOST_WITH_REASON:
      ...
      AJ_ErrPrintf(("AJ_StartClient(): AJ_SIGNAL_SESSION_LOST_WITH_REASON: "
                    "AJ_ERR_READ\n"));
      status = AJ_ERR_READ;
      break;
...
AJ_Status AJ_StartClient(AJ_BusAttachment *bus, const char *daemonName,
                         ...
                         const AJ_SessionOpts *opts) {
  return StartClient(bus, daemonName, timeout, connected, name, port, NULL,
                     sessionId, NULL, opts);
}
...
AJ_Status AJ_StartClientByInterface(AJ_BusAttachment *bus,
                                    ...
                                    const AJ_SessionOpts *opts) {
  return StartClient(bus, daemonName, timeout, connected, NULL, 0, interfaces,
                     sessionId, serviceName, opts);
}
