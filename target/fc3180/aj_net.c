...
AJ_Status AJ_Net_Send(AJ_IOBuffer *buf) {
  ...
  if (tx > 0) {
    ...
    if (ret == -1) {
      AJ_ErrPrintf(
          ("AJ_Net_Send(): esp_ip_send() failed. status=AJ_ERR_WRITE\n"));
      return AJ_ERR_WRITE;
    }
    buf->readPtr += ret;
  }
...
AJ_Status AJ_Net_Recv(AJ_IOBuffer *buf, uint32_t len, uint32_t timeout) {
  ...
  AJ_InfoPrintf(
      ("AJ_Net_Recv(buf=0x%p, len=%d., timeout=%d.)\n", buf, len, timeout));
  ...
  if (rx) {
    ...
    if ((ret == -1) || (ret == 0)) {
      AJ_ErrPrintf(
          ("AJ_Net_Recv(): esp_ip_recv() failed. status=AJ_ERR_READ\n"));
      status = AJ_ERR_READ;
    } else {
      buf->writePtr += ret;
    }
...
AJ_Status AJ_Net_Connect(AJ_NetSocket *netSock, uint16_t port, uint8_t addrType,
                         const uint32_t *addr) {
  AJ_InfoPrintf(
      ("AJ_Net_Connect(netSock=0x%p, port=%d., addrType=%d., addr=0x%p)\n",
       netSock, port, addrType, addr));
  AJ_InfoPrintf(
      ("AJ_Net_Connect(): Connect to \"%s:%u\"\n", AddrStr(*addr), port));
  int tcpSock = esp_ip_connect("TCP", AddrStr(*addr), port);
  if (tcpSock == INVALID_SOCKET) {
    AJ_ErrPrintf(
        ("AJ_Net_Connect(): esp_ip_connect() failed. status=AJ_ERR_CONNECT\n"));
...
void AJ_Net_Disconnect(AJ_NetSocket *netSock) { CloseNetSock(netSock); }
...
AJ_Status AJ_Net_SendTo(AJ_IOBuffer *buf) {
  ...
  if (tx > 0) {
    if (context->udpSock != INVALID_SOCKET) {
      ...
      if (ret == -1) {
        AJ_ErrPrintf(
            ("AJ_Net_SendTo(): esp_ip_send() failed. status=AJ_ERR_WRITE\n"));
        return AJ_ERR_WRITE;
      }
      buf->readPtr += ret;
    }
...
AJ_Status AJ_Net_RecvFrom(AJ_IOBuffer *buf, uint32_t len, uint32_t timeout) {
  ...
  AJ_InfoPrintf(
      ("AJ_Net_RecvFrom(buf=0x%p, len=%d., timeout=%d.)\n", buf, len, timeout));
  ...
  if (rc == 0) {
    AJ_InfoPrintf((
        "AJ_Net_RecvFrom(): esp_ip_poll() timed out. status=AJ_ERR_TIMEOUT\n"));
    return AJ_ERR_TIMEOUT;
  }
  ...
  if (rx) {
    ...
    if (ret == -1) {
      AJ_ErrPrintf(
          ("AJ_Net_RecvFrom(): esp_ip_recv() failed. status=AJ_ERR_READ\n"));
      status = AJ_ERR_READ;
    } else {
      buf->writePtr += ret;
      status = AJ_OK;
...
AJ_Status AJ_Net_MCastUp(AJ_NetSocket *netSock) {
  ...
  netContext.udpSock =
      esp_ip_connect("UDP", AJ_IPV4_MULTICAST_GROUP, AJ_UDP_PORT);

  if (netContext.udpSock != INVALID_SOCKET) {
    esp_ip_group_join(AJ_IPV4_MULTICAST_GROUP);
    AJ_IOBufInit(&netSock->rx, rxDataMCast, sizeof(rxDataMCast), AJ_IO_BUF_RX,
                 &netContext);
    netSock->rx.x1.recv = AJ_Net_RecvFrom;
    AJ_IOBufInit(&netSock->tx, txDataMCast, sizeof(txDataMCast), AJ_IO_BUF_TX,
                 &netContext);
