...
List *serial_recvlst() { return recvlst; }

#define __SERIAL_BUFFER_SIZE 4096

int serial_init(int port, int baudrate) {
  comInit();
  return comOpen(port, baudrate, DATABITS_8, STOPBITS_1, PARITY_NONE,
                 FLOWCONTROL_NONE, COM_READWRITE, __SERIAL_BUFFER_SIZE);
}
      ...
      if (recvlst) {
        ...
        while (x != 0) {
          if (strstr(x, "+CIPRD") == x) {
            ...
            if (ln > 0) {
              esp_recv_data *rd =
                  (esp_recv_data *)malloc(sizeof(esp_recv_data));
              rd->id = id;
              rd->len = ln;
...
int setipaddr(int com, const char *addr, const char *netmask,
              const char *gateway) {
  serial_write2(com, "AT+CIPSTA=\"%s\",\"%s\",\"%s\"\r", addr, netmask,
                gateway);
  return serial_find(com, 5, "OK", 0);
}
