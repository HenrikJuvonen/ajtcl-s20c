...
esp_status esp_wifi_scan() {
	...
	for (i = 0; i < __esp_aps_size; i++) {
		if (__esp_aps[i].ssid && strcmp(__esp_aps[i].ssid, "") != 0) {
			console_write("  %d %s %d\n", __esp_aps[i].ecn, __esp_aps[i].ssid, __esp_aps[i].rssi);
		}
	}
...
esp_status esp_ip_set(uint32_t *addr, uint32_t *netmask, uint32_t *gateway) {
	return setipaddr(__esp_com, AddrStr2(*addr), AddrStr2(*netmask), AddrStr2(*gateway)) ? ESP_OK : ESP_ERR_NULL;
}
...
esp_status esp_ip_acquire(uint32_t *addr, uint32_t *netmask, uint32_t *gateway, uint32_t timeout) {
	...
	for (i = 0; timeout == 0 || i < timeout/100; i++) {
		if (acquireipaddr(__esp_com) && esp_ip_get(addr, netmask, gateway) == ESP_OK) {
			return ESP_OK;			
		}
		delay(100);
	}
	...
	for (i = 0; i < __esp_fds_size; i++) {
		...
		while (x != 0) {
			if (strstr(x, "+CIPCREATE:") == x) {
				...
				serial_write2(__esp_com, "AT+CIPCONNECT=%d,\"%s\",%d\r", fd, addr, port);
				delay(500);
...
int32_t esp_ip_send(int32_t fd, const char* data, uint32_t length) {
	if (fd >= 0 && fd < __esp_fds_size && __esp_fds[fd]) {
		...
		int len = mysprintf(__esp_ip_send_buf, "AT+CIPSENDB=%d,%u,'%s'\r", fd, length, hex);
