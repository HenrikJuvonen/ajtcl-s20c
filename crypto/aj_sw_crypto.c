...
typedef struct { uint32_t fkey[MAX_SCHEDULE_LEN]; } AES_CTX;
...
static const uint32_t Rconst[12] = {0x1,  0x2,  0x4,  0x8,  0x10, 0x20,
                                    0x40, 0x80, 0x1b, 0x36, 0x6c, 0xd8};

#define round_column(x0, x1, x2, x3)                                           \
  ftable[ROW_0(x0)] ^ ROTL8(ftable[ROW_1(x1)]) ^ ROTL16(ftable[ROW_2(x2)]) ^   \
...
#define lastround_column(x0, x1, x2, x3)                                       \
  (uint32_t) sbox[ROW_0(x0)] ^ SHFL8(sbox[ROW_1(x1)]) ^                        \
      SHFL16(sbox[ROW_2(x2)]) ^ SHFL24(sbox[ROW_3(x3)])
...
static uint32_t SubBytes(uint32_t a) {
  return (uint32_t)sbox[(uint8_t)(a)] | (sbox[(uint8_t)(a >> 8)] << 8) |
         (sbox[(uint8_t)(a >> 16)] << 16) | (sbox[(uint8_t)(a >> 24)] << 24);
}
...
void AJ_AES_Disable(void) { memset(&aes_context, 0, sizeof(aes_context)); }
...
void AJ_AES_CTR_128(const uint8_t *key, const uint8_t *in, uint8_t *out,
  ...
  while (len) {
    ...
    uint8_t enc[16];
    Unpack32(enc, tmp);
    uint8_t *p = (uint8_t *)enc;
    while (n--) {
      *out++ = *p++ ^ *in++;
      //			printf("Y out: %u %u %u -- %u\n", *(out-1),
      //*(p-1), *(in-1), len);
    }
    /*
     * The counter field is big-endian (dumb idea given everything else is
     * processed little endian)
     */
    //#if HOST_IS_LITTLE_ENDIAN
    /*
...
void AJ_AES_ECB_128_ENCRYPT(const uint8_t *key, const uint8_t *in,
                            uint8_t *out) {
  uint32_t in32[4];
  uint32_t out32[4];
