...
/**
 * Callback function prototype for requesting a password or pincode from an
 * application.
 *
 * @param buffer  The buffer to receive the password.
 * @param bufLen  The size of the buffer
 *
 * @return  Returns the length of the password. If the length is zero this will
 * be
 ...
 */
typedef AJ_Status (*AJ_AuthListenerFunc)(uint32_t authmechanism,
                                         uint32_t command,
                                         AJ_Credential *creds);
...
typedef struct _AJ_BusAttachment {
  ...
  AJ_AuthListenerFunc
      authListenerCallback; /**< Callback for obtaining passwords */
...
#define AJ_NAME_REQ_ALLOW_REPLACEMENT                                          \
  0x01 /**< Allow others to take ownership of this name */
#define AJ_NAME_REQ_REPLACE_EXISTING                                           \
  0x02 /**< Attempt to take ownership of name if already taken */
#define AJ_NAME_REQ_DO_NOT_QUEUE                                               \
  0x04 /**< Fail if name cannot be immediately obtained */
 ...
 */
AJ_EXPORT
AJ_Status AJ_BusRequestName(AJ_BusAttachment *bus, const char *name,
                            uint32_t flags);

#define AJ_TRANSPORT_NONE 0x0000 /**< no transports */
#define AJ_TRANSPORT_ALL                                                       \
  0xFFFF /**< ALL Possible transports including EXPERIMENTAL ones */
...
#define AJ_TRANSPORT_PROXIMITY                                                 \
  0x0040 /**< Transport compatible with WinRT Proximity Framework */

#define AJ_TRANSPORT_TCP                                                       \
  0x0004 /**< Transport using TCP (same thing as WLAN that implies TCP) */
#define AJ_TRANSPORT_UDP                                                       \
  0x0100 /**< Transport using the AllJoyn Reliable Datagram Protocol (flavor \ \
            \                                                                  \
            of reliable UDP) */
#define AJ_TRANSPORT_IP                                                        \
  (AJ_TRANSPORT_TCP |                                                          \
   AJ_TRANSPORT_UDP) /**< Let the system decide which to use */

#define AJ_TRANSPORT_ANY                                                       \
  (AJ_TRANSPORT_ALL &                                                          \
   ~AJ_TRANSPORT_UDP) /**< ANY non-EXPERIMENTAL transport \ \                                                                             \
                         */
 ...
 * @param transportMask Restricts the transports the advertisement will be
 * stopped/started on.
 * @param op            Either AJ_BUS_START_ADVERTISING or
 * AJ_BUS_STOP_ADVERTISING
 ...
 */
AJ_EXPORT
AJ_Status AJ_BusAdvertiseName(AJ_BusAttachment *bus, const char *name,
                              uint16_t transportMask, uint8_t op,
                              uint8_t flags);
...
#define AJ_FIND_NAME_ALREADY                                                   \
  0x2 /**< Was already finding the requested name   \ \                                                                             \
         */
 ...
 *
 * @param  bus          The bus attachment
 * @param  namePrefix   Well-known name prefix that application is interested in
 * receiving
 ...
 */
AJ_EXPORT
AJ_Status AJ_BusFindAdvertisedName(AJ_BusAttachment *bus,
                                   const char *namePrefix, uint8_t op);
 ...
 */
AJ_EXPORT
AJ_Status AJ_BusFindAdvertisedNameByTransport(AJ_BusAttachment *bus,
                                              const char *namePrefix,
                                              uint16_t transport, uint8_t op);

#define AJ_SESSION_PROXIMITY_ANY 0xFF /**< No proximity restrictions */
#define AJ_SESSION_PROXIMITY_PHYSICAL                                          \
  0x01 /**< Limit to services that are physically proximal */
#define AJ_SESSION_PROXIMITY_NETWORK                                           \
  0x02 /**< Allow services that are on the same subnet */

#define AJ_SESSION_TRAFFIC_MESSAGES                                            \
  0x01 /**< Session carries message traffic  \ \                                                                             \
          */
#define AJ_SESSION_TRAFFIC_RAW_UNRELIABLE                                      \
  0x02 /**< Not supported by this implementation */
#define AJ_SESSION_TRAFFIC_RAW_RELIABLE                                        \
  0x04 /**< Not supported by this implementation */

#define AJ_SESSION_PORT_ANY                                                    \
  0x00 /**< Use a daemon assigned ephemeral session port */
 ...
 *
 * @param bus          The bus attachment
 * @param port         The port to bind, if AJ_SESSION_PORT_ANY is passed in,
 * the daemon
 *                     will assign an ephemeral session port
 * @param opts         Options for establishing a session, if NULL defaults are
 * used.
 * @param flags        Flags to pass into AJ_MarshalMsg
 *
 * @return  Return AJ_Status
 ...
 */
AJ_EXPORT
AJ_Status AJ_BusBindSessionPort(AJ_BusAttachment *bus, uint16_t port,
                                const AJ_SessionOpts *opts, uint8_t flags);
 ...
 */
#define AJ_CANCELSESSIONLESS_REPLY_SUCCESS                                     \
  1 /**< Cancel Sessionless: reply success */
#define AJ_CANCELSESSIONLESS_REPLY_NO_SUCH_MSG                                 \
  2 /**< Cancel Sessionless: no such msg */
#define AJ_CANCELSESSIONLESS_REPLY_NOT_ALLOWED                                 \
  3 /**< Cancel Sessionless: not allowed */
#define AJ_CANCELSESSIONLESS_REPLY_FAILED                                      \
  4 /**< Cancel Sessionless: reply failed */
 ...
 *          - AJ_ERR_MARSHAL if the message arguments were incompletely
 * marshaled
 */
AJ_EXPORT
 ...
 *
 * @param bus          The bus attachment
 * @param sessionHost  Bus name of attachment that is hosting the session to be
 * joined.
 ...
 */
AJ_EXPORT
AJ_Status AJ_BusJoinSession(AJ_BusAttachment *bus, const char *sessionHost,
                            uint16_t port, const AJ_SessionOpts *opts);
...
/**
 * Add a SIGNAL match rule. A rule must be added for every non-session signal
 ...
 */
AJ_EXPORT
AJ_Status AJ_BusSetSignalRule(AJ_BusAttachment *bus, const char *ruleString,
                              uint8_t rule);
 ...
 */
AJ_EXPORT
AJ_Status AJ_BusAddSignalRule(AJ_BusAttachment *bus, const char *signalName,
                              const char *interfaceName, uint8_t rule);
 ...
 */
AJ_EXPORT
AJ_Status AJ_BusSetSignalRuleFlags(AJ_BusAttachment *bus,
                                   const char *ruleString, uint8_t rule,
                                   uint8_t flags);

#define AJ_SETLINKTIMEOUT_SUCCESS 1 /**< SetLinkTimeout reply: Success */
#define AJ_SETLINKTIMEOUT_NO_DEST_SUPPORT                                      \
  2 /**< SetLinkTimeout reply: Destination endpoint does not support link \ \                                                                             \
       monitoring */
#define AJ_SETLINKTIMEOUT_NO_SESSION                                           \
  3 /**< SetLinkTimeout reply: Session with given id does not exist */
#define AJ_SETLINKTIMEOUT_FAILED 4 /**< SetLinkTimeout reply: Failed */

/**
 * Set a link timeout on a session. This will ensure that a session lost signal
 * is reported by the
 * daemon within the specified timeout period if the session peer unexpectedly
 * leaves the bus, for
 * example because the peer moved out of range. The application may want to
 * handle to reply to
 ...
 */
AJ_EXPORT
AJ_Status AJ_BusSetLinkTimeout(AJ_BusAttachment *bus, uint32_t sessionId,
                               uint32_t linkTimeout);
/*
 * Use to remove a member from the session
 *
 * @param bus           The bus attachment your on
 * @param sessionId     The session ID for the session your removing the member
 * from
 * @param member        The unique ID of the member you wish to remove from the
 * session
 *
 * @return  Return AJ_Status
 ...
 */
AJ_EXPORT
AJ_Status AJ_BusRemoveSessionMember(AJ_BusAttachment *bus, uint32_t sessionId,
                                    const char *member);
/*
...
/**
 * Invoke a built-in handler for standard bus messages. Signals passed to this
 * function that are
 * not bus messages are silently ignored. Method calls passed to this function
 * that are not
...
/**
 * Set a callback for returning passwords for peer authentication.
 ...
 */
AJ_EXPORT
void AJ_BusSetPasswordCallback(AJ_BusAttachment *bus,
 ...
 */
void AJ_BusSetAuthListenerCallback(AJ_BusAttachment *bus,
 ...
 * @param callback       A function to be called when the authentication
 * completes
 * @param cbContext      A caller provided context to pass to the callback
 * function
 *
 * @return  Return AJ_Status
 *         - AJ_OK if the request was sent
 *         - An error status otherwise
 */
AJ_Status AJ_BusAuthenticatePeer(AJ_BusAttachment *bus, const char *peerBusName,
                                 AJ_BusAuthPeerCallback callback,
                                 void *cbContext);

/**
 * Callback function prototype for a callback function to GET a property. All
 * this function has to
 * do is marshal the property value.
 *
 * @param replyMsg  The GET_PROPERTY reply message
 * @param propId    The property identifier
 * @param context   The caller provided context that was passed into
 * AJ_BusPropGet()
 *
 * @return  Return AJ_Status
 *          - AJ_OK if the property was read and marshaled
 *          - An error status if the property could not be returned for any
 * reason.
 */
typedef AJ_Status (*AJ_BusPropGetCallback)(AJ_Message *replyMsg,
                                           uint32_t propId, void *context);

/**
 * Helper function that provides all the boilerplate for responding to a
 * GET_PROPERTY. All the
 * application has to do is marshal the property value.
 *
 * @param msg       An unmarshalled GET_PROPERTY message
 * @param callback  The function called to request the application to marshal
 * the property value.
 * @param context   A caller provided context that is passed into the callback
 * function
 *
 * @return  Return AJ_Status
 */
AJ_EXPORT
AJ_Status AJ_BusPropGet(AJ_Message *msg, AJ_BusPropGetCallback callback,
                        void *context);

/**
 * Callback function prototype for a callback function to SET an application
 * property. All this
 ...
 */
typedef AJ_Status (*AJ_BusPropSetCallback)(AJ_Message *replyMsg,
                                           uint32_t propId, void *context);

/**
 * Helper function that provides all the boilerplate for responding to a
 * SET_PROPERTY. All the
 ...
 */
AJ_EXPORT
AJ_Status AJ_BusPropSet(AJ_Message *msg, AJ_BusPropSetCallback callback,
                        void *context);
 ...
 * @return  Return AJ_Status
 */
AJ_Status AJ_BusEnableSecurity(AJ_BusAttachment *bus, const uint32_t *suites,
                               size_t numsuites);
