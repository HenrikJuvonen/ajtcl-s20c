...
#define AJ_NVRAM_ID_CREDS_MAX                                                  \
  0x0FFF /**< Last NVRAM ID reserved for AllJoyn credentials management */
#define AJ_NVRAM_ID_RESERVED_MAX                                               \
  0x7FFF /**< Last NVRAM ID reserved for AllJoyn framework and services use */
#define AJ_NVRAM_ID_FOR_APPS                                                   \
  0x8000 /**< First NVRAM ID available for application used */
...
typedef struct _AJ_NV_DATASET {
  ...
  uint16_t capacity; /**< The capacity of the data set established by
                        AJ_NVRAM_Open() */
  uint16_t id;       /**< The unique id of a data set */
  uint8_t *inode;    /**< Pointer or offset to a location of the data set in the
                        NVRAM */
 ...
 *    "w"  : write: Create an empty data set for output operations. If a data
 * set with the same id already exists, its contents are discarded.
 * @param capacity The reserved space size for the data set. Only used for "w"
 * access mode.
 *
 * @return A handle that specifies the data set. NULL if the open operation
 * fails.
 */
 ...
 *
 * @param ptr   Pointer to a block of memory with a size of at least size bytes
