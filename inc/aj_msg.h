...
#define AJ_ARG_OBJ_PATH                                                        \
  'o' /**< AllJoyn Name of an AllJoyn object instance basic type */
#define AJ_ARG_UINT16 'q' /**< AllJoyn 16-bit unsigned integer basic type */
#define AJ_ARG_STRING                                                          \
...
#define AJ_ARG_DICT_ENTRY                                                      \
  '{' /**< AllJoyn dictionary or map container type - an array of key-value \  \
         \                                                                     \
         pairs */
...
#define AJ_FLAG_ALLOW_REMOTE_MSG                                               \
  0x04 /**< Allow messeages from remote hosts  \ \                                                                             \
          */
...
#define AJ_HDR_MEMBER                                                          \
  0x03 /**< Member (message/signal) name header field type */
...
#define AJ_HDR_HANDLES                                                         \
  0x09 /**< Number of file/socket handles that accompany the message */
#define AJ_HDR_TIMESTAMP                                                       \
  0x10 /**< Time stamp header field type (AllJoyn specific headers start at \  \
          \                                                                    \
          0x10. Time stamp header field type) */
#define AJ_HDR_TIME_TO_LIVE                                                    \
  0x11 /**< Messages time-to-live header field type  \ \                                                                             \
          */
#define AJ_HDR_COMPRESSION_TOKEN                                               \
  0x12 /**< Messages compression token header field type */
...
struct _AJ_Arg {
  ...
  uint8_t flags;  /**< non-zero if the value is a variant - values > 1 indicate
                     variant-of-variant etc. */
  ...
  union {
    ...
    const char
        *v_objPath; /**< objPath(char *) type field value in the message */
    const char
        *v_signature; /**< signature(char *) type field value in the message */
...
struct _AJ_Message {
  ...
  uint8_t varOffset;     /**< For variant marshalling/unmarshalling - Offset to
                            start of variant signature */
...
/**
 * Unmarshals a message returning a message structure. Note that if a message is
 * received but is not
 * recognized this function will return an uninitialized msg with msgId == 0.
 ...
 *
 * @return
 *          - AJ_OK if a message header was succesfully unmarshaled. Note that
 * the message may have
 *            been consumed or rejected internally in which case the msgId will
 * be zero.
...
/**
 * Unmarshals the next argument from a message or next element in a container
 * (array, struct,
 * dictionary entry, or variant).
 *
 * @param msg     A pointer to a message that was unmarshaled by an earlier call
 * to AJ_UnmarshalMsg
 ...
 *          - AJ_ERR_READ if there was a read failure
 *          - AJ_ERR_NO_MORE when there is no more to unmarshal (typically for
...
/**
 * Unmarshals and discard the next argument from a message or next element in a
 * container (array,
 * struct, dictionary entry). Variants must be skipped atomically, that is
 * AJ_UnmarshalVariant()
...
/**
 * Attempts to reset the state of message so the arguments can be unmarshaled
 * again. All of the
 * arguments must have been unmarshaled, a restriction to ensure that endianness
 * conversions
 * are correct. This function allows a message handler to inspect the message
 * arguments and
 * decide whether to handle it or not. For example a service handler might need
 * to check if it
 ...
 *
 * @param msg       A pointer to a message that was unmarshaled by an earlier
 * call to AJ_UnmarshalMsg
 * @param signature The signature of the argument list to unmarshal.
 * @param ...       Pointers to values of the correct sizeo and type per the
 * signature.
 *
 * @return
 ...
 *
 * The main use of this function is for unmarshalling message payloads that
 * exceed the size of the
 * network transmit buffer. Note that the data pointer returned is only valid
...
/**
 * Prepare to unmarshal a variant. The next argument unmarshalled is expected to
 * be variant.
 *
 * @param msg   A pointer to the message currently being marshaled
...
/**
 * Closes an ummarshalled message when it is no longer needed. This releases
 * resources and makes the
 * bus available for unmarshalling another message. After a message has been
 * closed unmarshaled
 * values are no longer valid so this function should not be called until the
 * message and its
 ...
 * @param timeout      Time in milliseconds to allow for a reply to the message
 * before reporting
 *                     a timeout error message is reported to the application.
 *
 * @return
 *          - AJ_OK if a message header was succesfully marshaled
 *          - AJ_ERR_RESOURCES if the message is too big to marshal into the
 * message buffer
 ...
 * @param sessionId    The session this message is for.
 * @param flags        A logical OR of the applicable message flags
 * @param ttl          Time to live for this signal in milliseconds. This
...
AJ_Status AJ_MarshalSignal(AJ_BusAttachment *bus, AJ_Message *msg,
                           uint32_t msgId, const char *destination,
                           AJ_SessionId sessionId, uint8_t flags, uint32_t ttl);
 ...
 * Initialize and marshal a message that is a error response to a method call.
 * This is a wrapper
 * function around AJ_MarshalErrorMsg that chooses an appropriate error message
 * depending on the
...
/**
 * This function does partial delivery of a marshalled message. This allow an
 * application to send
 * messages that are larger (much larger) than the transmit buffer. The
 * remaining data must be
 * custom marshaled by the application using AJ_MarshalRaw(). After all the
 * remaining data has been
 * marshaled the applicatiom must call AJ_DeliverMsg() to complete the delivery
 * of the message to
 * the network.
 *
 *
...
/**
 * Marshals one or arguments of basic types such as integers, strings, etc.
 * Container types
 ...
 *
 * - Scalar values (boolean, bytes, signed and unsigned integer of various
 * sizes, and doubles)
 ...
 * @param flags   Indicates if the argument is an array. Valid values are
 * AJ_ARRAY_FLAG and 0
 * @param val     The value to set, a string pointer or an address
 * @param len     The length of the value if flags is AJ_ARRAY_FLAG or 0
 * otherwise
 *
 ...
 *
 * @param msg     A pointer to a message that has marshaled by an earlier call
 * to AJ_MarshalMsg
 * @param arg     The argument to marshal
 *
 * @return
 *          - AJ_OK if the argument was succesfully marshaled.
 *          - AJ_ERR_RESOURCES if the arg is too big to marshal into the message
 * buffer
 *          - AJ_ERR_WRITE if there was a write failure
 */
AJ_EXPORT
...
/**
 * Marshals data for a message as raw bytes. The application is responsible for
 * correctly composing
 * the data according to the wire protocol specification including any padding
 * that may be required
 * as required by the alignment rules.
 *
 * The main use of this function is for marshalling message payloads that exceed
 * the size of the
 * network transmit buffer. Before calling this function the application will
 * typically call
 * AJ_DeliverMsgPartial() to establish the total length of the message in the
 * message header.
 *
 * The simple uses cases are marshaling of long strings and arrays of bytes or
 * integers.
 * In these two cases the application must marshal a 32 bit length then then
 * marshal the data.
 *
 * Note that strings must be NUL terminated but NUL is not included in the
 * length.
 *
 * @param msg   A pointer to the message currently being marshaled
...
/**
 * Prepare to marshal a variant. The next argument marshalled will be marshalled
 * as a variant.
 *
 * @param msg   A pointer to the message currently being marshaled
