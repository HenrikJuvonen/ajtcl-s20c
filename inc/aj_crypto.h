...
/**
 * Implements AES-CCM (Counter with CBC-MAC) encryption as described in RFC
 * 3610. The message in
 * encrypted in place.
 *
 * @param key     The AES-128 encryption key
 * @param msg     The buffer containing the entire message that is to be
 * encrypted, The buffer must
 *                have room at the end to append an authentication tag of length
 * tagLen.
 * @param msgLen  The length of the entire message
 * @param hdrLen  The length of the header portion that will be authenticated
 * but not encrypted
 * @param tagLen  The length of the authentication tag to be appended to the
 * message
...
/**
 * Implements AES-CCM (Counter with CBC-MAC) decryption as described in RFC
 * 3610. The message in
 * decrypted in place.
 *
 * @param key     The AES-128 encryption key
...
/**
 * A pseudo-random function for generation of keying material. This function
 * uses AES-CCM to
 * as the MAC function.
 *
 ...
 *               - AJ_ERR_RESOURCES if the hexLen is too small to fit the
 * converted string.
 */
...
/**
 * AES counter mode encryption/decryption. Note that in CTR mode encrytion is
 * its own inverse.
 *
