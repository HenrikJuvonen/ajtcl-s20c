...
typedef struct _AJ_PeerCred {
  ...
  uint32_t expiration; /**< the expiry time expressed a number of seconds since
                          Epoch */
 ...
 * @param peerCredHolder Pointer to a credential object pointer.  This address
 * will hold the new allocated credential object for the specific remote peer
 * identified by a GUID
 *
 * @return
 *      - AJ_OK if the credentials for the specific remote peer exist and are
...
/**
 * Get the GUID for this peer. If this is the first time the GUID has been
 * requested this function
 * will generate the GUID and store it in NVRAM
 *
 * @param[out] localGuid Pointer to a bufffer that has enough space to store the
 * local GUID
 *
 ...
 * @param credHolder Pointer to a credential object pointer.  This address will
 * hold the new allocated credential object for the specific custom credential
 *
 * @return
...
AJ_Status AJ_GetCredential(const uint16_t credType, const uint8_t *id,
                           uint8_t idLen, AJ_PeerCred **credHolder);
 ...
 * @param credType  the credential type to search
 * @param id        the credential id to search
 * @param[out] credHolder Pointer to a credential object pointer.  This address
 * will hold the new allocated credential object for the specific custom
 * credential
 *
 * @return
 *      - AJ_OK if the credential is found
