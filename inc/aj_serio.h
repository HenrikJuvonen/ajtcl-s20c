...
#define AJ_SERIO_RX                                                            \
  1 /**< The receive direction (from the wire) of the serial I/O subsystem */
#define AJ_SERIO_TX                                                            \
  2 /**< The transmit direction (to the wire) of the serial I/O subsystem */
...
typedef struct _AJ_SerIOConfig {
  ...
  uint8_t parity;   /**< Zero disables parity checking, one means odd and two
                       means even parity */
  void *config;     /**< Abstracted context for device/platform specific
                       configuration items */
} AJ_SerIOConfig;
 ...
 * system.
 *
 * @param direction  Provide AJ_SERIO_RX for the receiver section, AJ_SERIO_TX
 * for the transmitter.
 ...
 * @param tx_cb   The function to call when a frame has been completely
 * transmitted
 *
 * @return
 *          - AJ_OK              if the serial subsystem was successfully
 * initialized
