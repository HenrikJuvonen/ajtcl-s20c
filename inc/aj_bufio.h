 ...
 * @param len     The requested number of bytes to read. More or fewer bytes may
 * actually be read into
 *                the buffer.
 * @param timeout A timeout in milliseconds after which the read will return
 * with an error status if
 *                there is not data to read.
 *
 * @return
 *         - AJ_OK if some data was read
 *         - AJ_ERR_TIMEOUT if the read timedout
 *         - AJ_ERR_RESOURCES if there isn't enough room in the buffer to read
 * len bytes. The buffer
 ...
 */
typedef AJ_Status (*AJ_RxFunc)(struct _AJ_IOBuffer *buf, uint32_t len,
                               uint32_t timeout);
 ...
 */
#define AJ_IO_BUF_AVAIL(iobuf)                                                 \
  (uint32_t)(((iobuf)->writePtr - (iobuf)->readPtr))

/**
 * How much space is available to write to the buffer
 */
#define AJ_IO_BUF_SPACE(iobuf)                                                 \
  ((uint32_t)((iobuf)->bufSize - ((iobuf)->writePtr - (iobuf)->bufStart)))

/**
 * How much data has been consumed from the buffer
 */
#define AJ_IO_BUF_CONSUMED(iobuf)                                              \
  (uint32_t)(((iobuf)->readPtr - (iobuf)->bufStart))
 ...
 * @param direction Indicates if the buffer is being used for sending or
 * receiving data
