...
#define SLIPPED_LEN(packetsize)                                                \
  (((packetsize) + AJ_SERIAL_HDR_LEN + AJ_CRC_LEN) * 2 + AJ_BOUNDARY_BYTES)
 ...
 */
#define CNTRL_PACKET(t) (((t) == AJ_SERIAL_ACK) || ((t) == AJ_SERIAL_CTRL))

/**
 * Reliable packets are ACKed; unreliable packets are not.
 */
#define RELIABLE_PACKET(t) ((t) == AJ_SERIAL_DATA)
...
typedef struct _AJ_LinkParameters {
  ...
  uint32_t txResendTimeout; /**< how long to wait for an acknowledgement before
                               resending a packet */
  uint32_t txAckTimeout; /**< how long to wait after a packet has been received
                            before sending a ACK packet */
} AJ_LinkParameters;
 ...
 */
#define SEQ_GT(s1, s2)                                                         \
  (((7 + (s1) - (s2)) & 7) < AJ_SerialLinkParams.windowSize)

/**
 * CRC Computation. The CRC computation must be initialized to the following
 * value.
 */
 ...
 */
void AJ_SerialLinkPacket(uint8_t *buffer, uint16_t len);

/**
 * Function prototype for function called when the serial transport is open for
 * business.
 */
 ...
 * @param packetSize           Packet size with which the serial transport layer
 * is initialized
 */
...
/**
 * Send a data buffer over the serial transport. This function blocks until the
 ...
 */
AJ_Status AJ_SerialSend(uint8_t *buffer, uint16_t len);

/**
 * Receive data from the serial transport. This function blocks until the
 ...
 */
AJ_Status AJ_SerialRecv(uint8_t *buffer, uint16_t len, uint32_t timeout,
                        uint16_t *recv);
