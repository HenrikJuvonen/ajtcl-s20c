 ...
 * Type for a GUID
 */
typedef struct _AJ_GUID { uint8_t val[16]; /**< string for a GUID */ } AJ_GUID;
...
#define AJ_ROLE_KEY_UNDEFINED                                                  \
  0 /**< Indicates the session key role is undefined (only applies for group \ \
       \                                                                       \
       keys) */
#define AJ_ROLE_KEY_INITIATOR                                                  \
  1 /**< Indicates the session key was initiated by this peer */
#define AJ_ROLE_KEY_RESPONDER                                                  \
  2 /**< Indicates the session key was initiated by the remote peer */
 ...
 */
AJ_Status AJ_SetSessionKey(const char *uniqueName, const uint8_t *key,
                           uint8_t role);
 ...
 *
 * @param name       The unique or well-known name for a remote peer or NULL to
 * get the local group key.
