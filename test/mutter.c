...
static AJ_Status MsgInit(AJ_Message *msg, uint32_t msgId, uint8_t msgType) {
  msg->x1.objPath = "/test/mutter";
  msg->iface = "test.mutter";
  msg->x2.member = "mumble";
  msg->msgId = msgId;
...
static const char *const Fruits[] = {"apple",      "banana", "cherry", "durian",
                                     "elderberry", "fig",    "grape"};

static const char *const Colors[] = {"azure", "blue", "cyan", "dun", "ecru"};

static const uint8_t Data8[] = {0xA0, 0xB0, 0xC0, 0xD0, 0xE0,
                                0xF0, 0xA1, 0xB1, 0xC2, 0xD3};
static const uint16_t Data16[] = {0xFF01, 0xFF02, 0xFF03,
                                  0xFF04, 0xFF05, 0xFF06};
static const char *const string_marshalled_after_scalar_array =
    "string after array";
int AJ_Main_mutter() {
  AJ_Status status;
 ...
 */
  ...
  for (test = 0; test < ArraySize(testSignature); ++test) {
    ...
    case 0:
      status = AJ_MarshalArgs(&txMsg, "ays", Data8, sizeof(Data8),
                              string_marshalled_after_scalar_array);
      if (status != AJ_OK) {
        break;
      }
      break;
    ...
    case 3:
      ...
      for (u = 0; u < ArraySize(Fruits); ++u) {
        ...
        status = AJ_MarshalArg(
            &txMsg, AJ_InitArg(&arg, AJ_ARG_BYTE, AJ_ARRAY_FLAG, Data8, u));
        if (status != AJ_OK) {
          break;
        }
        status = AJ_MarshalCloseContainer(&txMsg, &struct1);
...
#else
      status = AJ_MarshalArgs(&txMsg, "v", "(ivi)", 121212121, "s",
                              "inner variant", 3434343);
      if (status != AJ_OK) {
        break;
      }
#endif
    ...
    case 8:
      ...
      status =
          AJ_MarshalArg(&txMsg, AJ_InitArg(&arg, AJ_ARG_BYTE, AJ_ARRAY_FLAG,
                                           Data8, sizeof(Data8)));
      if (status != AJ_OK) {
        break;
      }
      ...
      status =
          AJ_MarshalArg(&txMsg, AJ_InitArg(&arg, AJ_ARG_UINT16, AJ_ARRAY_FLAG,
...
#else
      status =
          AJ_MarshalArgs(&txMsg, "(vvvvvv)", "i", 121212121, "s", "variant",
                         "ay", Data8, sizeof(Data8), "ay", Data8, sizeof(Data8),
                         "aq", Data16, sizeof(Data16), "s", "variant2");
      if (status != AJ_OK) {
        break;
      }
#endif
...
#else
        if (j == 4) {
          status = AJ_MarshalArgs(&txMsg, "{iv}", j, "s",
                                  "This is a variant string");
          if (status != AJ_OK) {
            break;
          }
        } else {
    ...
    case 0:
      ...
      if (strncmp(str, string_marshalled_after_scalar_array,
    ...
    case 10:
      ...
      for (j = 0; j < len; ++j) {
        TestStruct *ts;
        status = AJ_UnmarshalRaw(&rxMsg, (const void **)&ts, sizeof(TestStruct),
                                 &sz);
        if (status != AJ_OK) {
          break;
        }
        if ((ts->a != j) || (ts->b != (j + 1)) || (ts->c != (j + 2)) ||
            (ts->d != (j + 3))) {
          status = AJ_ERR_FAILURE;
          break;
        }
    ...
    case 15:
      ...
      for (j = 0;; ++j) {
        ...
        if (key == 4) {
          ...
          AJ_AlwaysPrintf(
              ("Unmarshal dict entry key=%d variant %s\n", key, sig));
  ...
  if (status != AJ_OK) {
    AJ_AlwaysPrintf(
        ("Marshal/Unmarshal unit test[%d] failed %d\n", test, status));
  }
...
#ifdef AJ_MAIN
int mutter() { return AJ_Main_mutter(); }
#endif
