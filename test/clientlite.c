 ...
 * To define a secure interface, prepend '$' before the interface name, eg.,
 * "$org.alljoyn.alljoyn_test"
 */
#ifdef SECURE_INTERFACE
static const char testInterfaceName[] = "$org.alljoyn.alljoyn_test";
static const char testValuesInterfaceName[] =
...
static const char *testInterfaceNames[] = {testInterfaceName,
                                           testValuesInterfaceName, NULL};
#endif

static const char *const testInterface[] = {testInterfaceName,
                                            "?my_ping inStr<s outStr>s", NULL};

static const char *const testValuesInterface[] = {testValuesInterfaceName,
                                                  "@int_val=i", NULL};

static const AJ_InterfaceDescription testInterfaces[] = {
    AJ_PropertiesIface, testInterface, testValuesInterface, NULL};
...
static AJ_Object ProxyObjects[] = {
    {NULL, testInterfaces,
     AJ_OBJ_FLAG_SECURE}, /* Object path will be specified later */
    {NULL}};
#else
static AJ_Object ProxyObjects[] = {
 ...
 */
static AJ_Status SendPing(AJ_BusAttachment *bus, uint32_t sessionId,
                          const char *serviceName, unsigned int num);
...
static const char owner_pub_b64[] = "RCf5ihem02VFXvIa8EVJ1CJcJns3el0IH+"
                                    "H51s07rc0AAAAAn6KJifUPH1oRmPLoyBHGCg7/"
                                    "NT8kW67GD8kQjZh/U/AAAAAAAAAAAA==";
static const char ecc_pub_b64[] = "JmZC779f7YjYPa3rU0xdifnW0qyiCmmUXcN1XExC334A"
                                  "AAAA1j95MCfIAFa6Fpa5vJ+"
                                  "2tUMfYVmhny04itEwJPnfDqAAAAAAAAAAAA==";
static const char ecc_prv_b64[] =
    "koWEteat13YRYrv/olCqEmMg7YufcTsjSQNbIL1ue+wAAAAA";
...
static const char *issuers[] = {"RCf5ihem02VFXvIa8EVJ1CJcJns3el0IH+"
                                "H51s07rc0AAAAAn6KJifUPH1oRmPLoyBHGCg7/"
                                "NT8kW67GD8kQjZh/U/AAAAAAAAAAAA==",
                                "9RB2ExIO4VZqEwb+"
                                "sWYVsozToGMgDZJzH0Yf4Q0sCC0AAAAAhuEeeMDIXKzoOg"
                                "3aQqVdUKC0ekWIRizM5hcjzxAO8LUAAAAAAAAAAA=="};
...
static AJ_Status AuthListenerCallback(uint32_t authmechanism, uint32_t command,
                                      AJ_Credential *cred) {
...
static AJ_Status SendPing(AJ_BusAttachment *bus, uint32_t sessionId,
  ...
  /*
   * Since the object path on the proxy object entry was not set in the proxy
   * object table above
   * it must be set before marshalling the method call.
   */
  status = AJ_SetProxyObjectPath(ProxyObjects, PRX_MY_PING, testObj);
  if (status == AJ_OK) {
    status = AJ_MarshalMethodCall(bus, &msg, PRX_MY_PING, serviceName,
...
AJ_Status SendGetProp(AJ_BusAttachment *bus, uint32_t sessionId,
  ...
  status = AJ_MarshalMethodCall(bus, &msg, PRX_GET_PROP, serviceName, sessionId,
                                0, METHOD_TIMEOUT);
  if (status == AJ_OK) {
...
AJ_Status SendSetProp(AJ_BusAttachment *bus, uint32_t sessionId,
                      const char *serviceName, int val) {
  AJ_Status status;
  AJ_Message msg;

  status = AJ_MarshalMethodCall(bus, &msg, PRX_SET_PROP, serviceName, sessionId,
                                0, METHOD_TIMEOUT);
  if (status == AJ_OK) {
    ...
    if (status == AJ_OK) {
      ...
      AJ_AlwaysPrintf((">>>>>>>>In SendSetProp() AJ_MarshalPropertyArgs() "
                       "returned status = 0x%04x\n",
                       status));
    }
    ...
    if (status == AJ_OK) {
      ...
      AJ_AlwaysPrintf((">>>>>>>>In SendSetProp() AJ_MarshalArgs() returned "
                       "status = 0x%04x\n",
                       status));
    }
  }
...
int AJ_Main_clientlite(int ac, char **av)
#else
int AJ_Main_clientlite()
#endif
{
  ...
  while (TRUE) {
...
#ifndef NGNS
      status =
          AJ_StartClient(&bus, NULL, CONNECT_TIMEOUT, FALSE, testServiceName,
                         testServicePort, &sessionId, NULL);
#else
      status = AJ_StartClientByInterface(&bus, NULL, CONNECT_TIMEOUT, FALSE,
                                         testInterfaceNames, &sessionId,
                                         testServiceName, NULL);
#endif
      if (status == AJ_OK) {
        AJ_AlwaysPrintf(
            ("StartClient returned %d, sessionId=%u, serviceName=%s\n", status,
             sessionId, testServiceName));
        ...
        status = AJ_BusAuthenticatePeer(&bus, testServiceName, AuthCallback,
                                        &authStatus);
        if (status != AJ_OK) {
    ...
    AJ_AlwaysPrintf(
        ("Auth status %d and AllJoyn status %d\n", authStatus, status));
    ...
    if (status != AJ_OK) {
      ...
      case AJ_REPLY_ID(AJ_METHOD_BUS_PING): {
        ...
        if (disposition == AJ_PING_SUCCESS) {
          ...
          AJ_AlwaysPrintf(
              ("Bus Ping failed, disconnecting: %d\n", disposition));
...
#ifdef AJ_MAIN
#ifdef MAIN_ALLOWS_ARGS
int clientlite(int ac, char **av) { return AJ_Main_clientlite(ac, av); }
#else
int clientlite() { return AJ_Main_clientlite(); }
#endif
#endif
