...
/**
 * Given a buffer of fixed length, randomly pick a way to corrupt the data (or
 * not.)
 */
void FuzzBuffer(uint8_t *buf, uint32_t len) {
  ...
  AJ_AlwaysPrintf(
      ("FuzzBuffer: Before case %i Tag:%c%c%c%c Ack:%i Seq:%i Type:%X Len:%i\n",
       test, ph->tag[0], ph->tag[1], ph->tag[2], ph->tag[3], ps->ack, ps->seq,
       ps->type, packetLen));
  ...
  AJ_AlwaysPrintf(
      ("FuzzBuffer: After  case %i Tag:%c%c%c%c Ack:%i Seq:%i Type:%X Len:%i\n",
       test, ph->tag[0], ph->tag[1], ph->tag[2], ph->tag[3], ps->ack, ps->seq,
       ps->type, packetLen));
...
#ifdef READTEST
    AJ_Sleep(2000); // wait for the writing side to be running, this should test
                    // the queuing of data.
