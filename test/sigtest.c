...
static const char *const testInterface[] = {
    "org.alljoyn.LargeSignals", "!large_signal >u >ay", "@ok_to_send=b", NULL};

static const AJ_InterfaceDescription testInterfaces[] = {
    testInterface, AJ_PropertiesIface, NULL};

/**
 * Objects implemented by the application
 */
static const AJ_Object ProxyObjects[] = {
    {"/org/alljoyn/LargeSignals", testInterfaces}, {NULL}};
...
AJ_Status AppHandleSetProp(AJ_Message *replyMsg, uint32_t propId,
                           void *context) {
...
void HandleSignal(AJ_Message *msg) {
  ...
  if (AJ_OK == status) {
    status = AJ_UnmarshalRaw(msg, (const void **)&raw,
                             sizeof(bytesUnmarshalled), &sz);
    if (AJ_OK != status) {
  ...
  if (AJ_OK == status) {
    ...
    if (firstByte != lastByte) {
      AJ_Printf(
          "ERROR: Integrity check failed. first element != last element. \n");
      g_error = 1;
    }

    AJ_Printf("============> Received my_signal # %u, length %u, firstByte %u, "
              "lastByte %u\n",
...
static AJ_Status SendSignal(AJ_BusAttachment *bus, uint32_t sessionId) {
  ...
  status =
      AJ_MarshalSignal(bus, &msg, APP_MY_SIGNAL, ServiceName, sessionId, 0, 0);
  if (AJ_OK == status) {
    ...
    if (ARRAY_SIZE < (array_size - 2)) {
      for (i = 0; i < (array_size - 2) / ARRAY_SIZE; i++) {
        ...
        if (AJ_OK != status) {
          AJ_Printf("ERROR: 1 Marshaling element (index: %d) failed. Got "
                    "status: %s.\n",
    ...
    if (0 != (array_size - 2) % ARRAY_SIZE) {
      ...
      if (AJ_OK != status) {
        AJ_Printf(
            "ERROR: 2 Marshaling element (index: %d) failed. Got status: %s.\n",
            i, AJ_StatusText(status));
      }
    }
...
void AJ_Main_sigtest(void) {
  ...
  while (TRUE) {
    ...
    if (!connected) {
      status = AJ_StartClient(&g_bus, DaemonName, CONNECT_TIMEOUT, 0,
                              ServiceName, ServicePort, &g_sessionId, NULL);
      if (AJ_OK == status) {
