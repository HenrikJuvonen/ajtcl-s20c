...
int AJ_Main_certificate(int ac, char **av) {
  ...
  for (i = 0; i < num; i++) {
    ...
    status = AJ_CreateCertificate(cert, 1, &root_pubkey, &peer_pubkey, NULL,
                                  digest, 0);
    AJ_ASSERT(AJ_OK == status);
    ...
    status = AJ_CreateCertificate(cert, 2, &root_pubkey, &peer_pubkey, &guild,
                                  digest, 0);
    AJ_ASSERT(AJ_OK == status);
    status = AJ_SignCertificate(cert, &root_prvkey);
    AJ_ASSERT(AJ_OK == status);
...
#ifdef AJ_MAIN
int certificate(int ac, char **av) { return AJ_Main_certificate(ac, av); }
#endif
