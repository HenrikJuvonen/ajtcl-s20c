    ...
    while (TRUE) {
        ...
        if (!connected) {
            ...
            if (status == AJ_OK) {
                ...
                status = AJ_BusAuthenticatePeer(&bus, testServiceName, AuthCallback_siglite, &authStatus);
                if (status != AJ_OK) {
...
#ifdef AJ_MAIN
#ifdef MAIN_ALLOWS_ARGS
int siglite(int ac, char** av)
{
    return AJ_Main_siglite(ac, av);
}
#else
int siglite()
{
    return AJ_Main_siglite();
}
#endif
#endif
