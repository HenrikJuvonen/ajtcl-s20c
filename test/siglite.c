...
#else
static const char *testInterfaceNames[] = {testInterfaceName, NULL};
...
static const AJ_Object ProxyObjects[] = {
    {"/org/alljoyn/alljoyn_test", testInterfaces}, {NULL}};
...
static AJ_Status SendSignal(AJ_BusAttachment *bus, uint32_t sessionId,
  ...
  status =
      AJ_MarshalSignal(bus, &msg, PRX_MY_SIGNAL, serviceName, sessionId, 0, 0);
  if (status == AJ_OK) {
    AJ_Arg arg;
...
void AuthCallback_siglite(const void *context, AJ_Status status) {
  *((AJ_Status *)context) = status;
}

#ifdef MAIN_ALLOWS_ARGS
int AJ_Main_siglite(int ac, char **av)
#else
int AJ_Main_siglite()
#endif
{
  ...
  while (TRUE) {
    ...
    if (!connected) {
      ...
      if (status == AJ_OK) {
        ...
        status = AJ_BusAuthenticatePeer(&bus, testServiceName,
                                        AuthCallback_siglite, &authStatus);
        if (status != AJ_OK) {
...
#ifdef AJ_MAIN
#ifdef MAIN_ALLOWS_ARGS
int siglite(int ac, char **av) { return AJ_Main_siglite(ac, av); }
#else
int siglite() { return AJ_Main_siglite(); }
#endif
#endif
