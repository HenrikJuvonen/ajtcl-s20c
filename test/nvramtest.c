...
AJ_Status TestECCCreds() {
  ...
  status = AJ_StoreLocalCredential(AJ_CRED_TYPE_DSA_PRIVATE, privateKeyID,
                                   (uint8_t *)&privateKey, sizeof(privateKey),
                                   0xFFFFFFFF);
  if (AJ_OK != status) {
    AJ_AlwaysPrintf(("AJ_StoreLocalCredential failed = %d\n", status));
    return status;
  }
  status = AJ_GetLocalCredential(AJ_CRED_TYPE_DSA_PRIVATE, privateKeyID,
                                 &privateKeyCred);
  if (AJ_OK != status) {
  ...
  if (privateKeyCred->dataLen != sizeof(privateKey)) {
    AJ_AlwaysPrintf(
        ("Retrieved private key length %d does not match the original %zu\n",
  ...
  status = AJ_StoreLocalCredential(AJ_CRED_TYPE_DSA_PUBLIC, publicKeyID,
                                   (uint8_t *)&publicKey, sizeof(publicKey),
                                   0xFFFFFFFF);
  if (AJ_OK != status) {
    AJ_AlwaysPrintf(("AJ_StoreLocalCredential failed = %d\n", status));
    return status;
  }
  status = AJ_GetLocalCredential(AJ_CRED_TYPE_DSA_PUBLIC, publicKeyID,
                                 &publicKeyCred);
  if (AJ_OK != status) {
    AJ_AlwaysPrintf(("AJ_GetLocalCredential failed = %d\n", status));
    return status;
...
AJ_Status TestObsWrite() {
  ...
  char *ssid[] = {
      "abcdefghABCDEFGH", "aaaaaaaa", "bbbbbbbb",
      "cccccccc",         "dddddddd", "eeeeeeee",
      "ffffffff",         "gggggggg", "hhhhhhhh",
      "iiiiiiii",         "jjjjjjjj", "kkkkkkkk",
      "llllllll",         "mmmmmmmm", "nnnnnnnn",
      "oooooooo",         "pppppppp", "qqqqqqqq",
      "rrrrrrrr",         "ssssssss", "",
      "tttttttt",         "uuuuuuuu", "vvvvvvvv",
      "wwwwwwww",         "xxxxxxxx", "yyyyyyyy",
      "zzzzzzzz",         "11111111", "22222222",
      "33333333",         "44444444", "55555555",
      "66666666",         "77777777", "888888888888888888888888888888",
      "99999999",         "aaaa1111", "bbbb2222",
      "cccc3333",         "dddd4444", "TRTESTING123",
      "eeee5555",         "",         "TR-TESTING-43"};
  char pc[] = "aaaaabbbbbcccccAAAAABBBBBCCCCCzzzzzZZZZZ1111122222";
  size_t i;

  AJ_NVRAM_Layout_Print();

  // if( AJ_NVRAM_Exist(AJ_NVRAM_ID_CREDS_MAX + 100)){ //NEGATIVE TEST, ID
  // DOESN'T EXIST
  // if( AJ_NVRAM_Exist(AJ_NVRAM_ID_CREDS_MAX + 1)){ //PROPERTY STORE DEVICE ID
  if (AJ_NVRAM_Exist(AJ_NVRAM_ID_FOR_APPS)) {
    // nvramHandle = AJ_NVRAM_Open(100, "r", 0); //NEGATIVE TEST, OPEN INVALID
    // ID
    // nvramHandle = AJ_NVRAM_Open(AJ_NVRAM_ID_CREDS_MAX + 1, "r", 0);
    // //PROPERTY STORE DEVICE ID
    ...
    if (nvramHandle != NULL) {
      ...
      if (sizeRead != size) {
        ...
        AJ_InfoPrintf(
            ("Read Info values: state=%d, ssid=%s authType=%d pc=%s\n",
             info.state, info.ssid, info.authType, info.pc));
      }
    }
  }

  // nTest = AJ_NVRAM_Read(&info, size, nvramHandle); //NEGATIVE TEST, READ NULL
  // HANDLE
  // nTest = AJ_NVRAM_Write(&info, size, nvramHandle); //NEGATIVE TEST, WRITE TO
  // NULL HANDLE
...
#ifdef SHOW_REWRITES
      AJ_AlwaysPrintf(
          ("Going to write Info values: state=%d, ssid=%s authType=%d pc=%s\n",
           info.state, info.ssid, info.authType, info.pc));
#endif

      // nvramHandle = AJ_NVRAM_Open(AJ_NVRAM_ID_CREDS_MAX + 1, "w", 0);
      // //NEGATIVE TEST, OPEN 0 SIZE
      // nvramHandle = AJ_NVRAM_Open(AJ_NVRAM_ID_CREDS_MAX + 1, "t", size);
      // //NEGATIVE TEST, INVALID MODE
      // nvramHandle = AJ_NVRAM_Open(0, "w", size); //NEGATIVE TEST, OPEN 0 ID
      // nvramHandle = AJ_NVRAM_Open(AJ_NVRAM_ID_CREDS_MAX + 1, "w", size);
      // //PROPERTY STORE DEVICE ID
...
AJ_Status TestNvramWrite() {
  ...
  while (TRUE) {
    ...
    AJ_InfoPrintf(
        ("Dataset1 bytes: %u i: %u sizeof(i): %u capacity %u curPos %u\n",
    ...
    AJ_InfoPrintf(
        ("Dataset2 bytes: %u i: %u sizeof(i): %u capacity %u curPos %u\n",
    ...
    AJ_InfoPrintf(
        ("Dataset3 bytes: %u i: %u sizeof(i): %u capacity %u curPos %u\n",
         bytes2, i, sizeof(i), d2->capacity, d2->curPos));
    AJ_InfoPrintf(("LAYOUT AFTER WRITE\n"));
    AJ_NVRAM_Layout_Print();
    if (d3 != NULL) {
    ...
    AJ_InfoPrintf(
        ("Dataset4 bytes: %u i: %u sizeof(i): %u capacity %u curPos %u\n",
         bytes2, i, sizeof(i), d2->capacity, d2->curPos));
    AJ_InfoPrintf(("LAYOUT AFTER WRITE\n"));
    AJ_NVRAM_Layout_Print();
    if (d4 != NULL) {
...
AJ_Status TestNvramDelete() {
  ...
  if (tid1 % 2 == 1) {
    ...
    if (AJ_NVRAM_Exist(AJ_NVRAM_ID_FOR_APPS)) {
      ...
      // AJ_NV_DATASET* nvramHandle = AJ_NVRAM_Open(AJ_NVRAM_ID_CREDS_MAX + 1,
      // "w", size); //PROPERTY STORE DEVICE ID
      nvramHandle = AJ_NVRAM_Open(AJ_NVRAM_ID_FOR_APPS, "w", size);
      if (nvramHandle != NULL) {
      ...
      if (nvramHandle != NULL) {
        ...
        if (sizeRead != sizeRead) {
          ...
          AJ_AlwaysPrintf(
              ("Read Info values: state=%d, ssid=%s authType=%d pc=%s\n",
               emptyInfo.state, emptyInfo.ssid, emptyInfo.authType,
               emptyInfo.pc));
        }
      }
    }
...
#ifdef AJ_MAIN
int nvramtest() { return AJ_Main_nvramtest(); }
#endif
