...
AJ_Status AppSendChatSignal_sessions(AJ_BusAttachment* bus, uint32_t sessionId, const char* chatString, uint8_t flags, uint32_t ttl)
{
    ...
    while (TRUE) {
        ...
        if (AJ_GetCmdLine(buf, 1024) != NULL && strlen(buf) > 0) {
            ...
            if (0 == strcmp("startservice", command)) {
                ...
                status = AppSendChatSignal_sessions(&bus, 0, chatMsg, ALLJOYN_FLAG_SESSIONLESS, sendTTL);
                ...
                chatMsg = strtok(NULL, "\r\n");
                if (chatMsg) {
                    status = AppSendChatSignal_sessions(&bus, session, chatMsg, 0, sendTTL);
                }
