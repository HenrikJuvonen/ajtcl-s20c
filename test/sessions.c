...
/// globals
uint8_t connected_sessions = FALSE;
...
static const char *const testInterface[] = {"org.alljoyn.bus.test.sessions",
                                            "!Chat >s", NULL};

static const AJ_InterfaceDescription testInterfaces[] = {testInterface, NULL};

/**
 * Objects implemented by the application
 */
static const AJ_Object AppObjects[] = {{"/sessions", testInterfaces}, {NULL}};
 ...
 */
void AppDoWork_sessions() { AJ_InfoPrintf(("AppDoWork\n")); }
...
void AuthCallback_sessions(const void *context, AJ_Status status) {
  *((AJ_Status *)context) = status;
}

AJ_Status AppSendChatSignal_sessions(AJ_BusAttachment *bus, uint32_t sessionId,
                                     const char *chatString, uint8_t flags,
                                     uint32_t ttl) {
  AJ_Status status = AJ_OK;
  AJ_Message msg;

  status =
      AJ_MarshalSignal(bus, &msg, APP_CHAT_SIGNAL, NULL, sessionId, flags, ttl);
  if (status == AJ_OK) {
...
AJ_Status AppHandleChatSignal_sessions(AJ_Message *msg) {
  AJ_Status status = AJ_OK;
  char *chatString;
...
void Do_Connect_sessions(AJ_BusAttachment *bus) {
  while (!connected_sessions) {
    AJ_Status status;
    ...
    AJ_BusSetSignalRule(bus, "type='signal',member='NameOwnerChanged',"
                             "interface='org.freedesktop.DBus'",
                        AJ_BUS_SIGNAL_ALLOW);
  }
}

int AJ_Main_sessions() {
  AJ_Status status = AJ_OK;
  AJ_BusAttachment bus;
  uint32_t g_sessionId = 0l;
  ...
  while (TRUE) {
    ...
    if (AJ_GetCmdLine(buf, 1024) != NULL && strlen(buf) > 0) {
      ...
      if (0 == strcmp("startservice", command)) {
        ...
        status = AJ_StartService(&bus, NULL, CONNECT_TIMEOUT, TRUE, port, name,
                                 AJ_NAME_REQ_DO_NOT_QUEUE, &opts);
        ...
        status =
            AJ_BusFindAdvertisedName(&bus, namePrefix, AJ_BUS_START_FINDING);
        ...
        status =
            AJ_BusFindAdvertisedName(&bus, namePrefix, AJ_BUS_STOP_FINDING);
        ...
        status = AJ_BusFindAdvertisedNameByTransport(
            &bus, namePrefix, transport, AJ_BUS_START_FINDING);
        ...
        status = AJ_BusFindAdvertisedNameByTransport(
            &bus, namePrefix, transport, AJ_BUS_STOP_FINDING);
        ...
        status = AJ_BusAdvertiseName(&bus, name, transport,
        ...
        if (port == 0) {
          AJ_AlwaysPrintf(("Usage: bind <port> [isMultipoint] [traffic] "
                           "[proximity] [transports]\n"));
          continue;
        }
        token = strtok(NULL, " \r\n");
        if (token) {
        ...
        if (!name || (port == 0)) {
          AJ_AlwaysPrintf(("Usage: join <name> <port> [isMultipoint] [traffic] "
                           "[proximity] [transports]\n"));
          continue;
        }
        token = strtok(NULL, " \r\n");
        ...
        status = AppSendChatSignal_sessions(&bus, 0, chatMsg,
                                            ALLJOYN_FLAG_SESSIONLESS, sendTTL);
        ...
        if (chatMsg) {
          status =
              AppSendChatSignal_sessions(&bus, session, chatMsg, 0, sendTTL);
        }
        ...
        // AJ_AlwaysPrintf(("debug <module_name> <level>
        // - Set debug level for a module\n"));
        AJ_AlwaysPrintf(("startservice [name] [port] [isMultipoint] [traffic] "
                         "[proximity] [transports] - Startservice\n"));
        AJ_AlwaysPrintf(("requestname <name>                                   "
                         "         - Request a well-known name\n"));
        AJ_AlwaysPrintf(("releasename <name>                                   "
                         "         - Release a well-known name\n"));
        AJ_AlwaysPrintf(("bind <port> [isMultipoint] [traffic] [proximity] "
                         "[transports] - Bind a session port\n"));
        AJ_AlwaysPrintf(("unbind <port>                                        "
                         "         - Unbind a session port\n"));
        AJ_AlwaysPrintf(("advertise <name> [transports]                        "
                         "         - Advertise a name\n"));
        AJ_AlwaysPrintf(("canceladvertise <name> [transports]                  "
                         "         - Cancel an advertisement\n"));
        AJ_AlwaysPrintf(("find <name_prefix>                                   "
                         "         - Discover names that begin with prefix\n"));
        AJ_AlwaysPrintf(
            ("find2 <name_prefix> <transport>                               - "
             "Discover names that begin with prefix by specific transports\n"));
        AJ_AlwaysPrintf(
            ("cancelfind <name_prefix>                                      - "
             "Cancel discovering names that begins with prefix\n"));
        AJ_AlwaysPrintf(("cancelfind2 <name_prefix> <transport>                "
                         "         - Cancel discovering names that begins with "
                         "prefix by specific transports\n"));
        AJ_AlwaysPrintf(("join <name> <port> [isMultipoint] [traffic] "
                         "[proximity] [transports] - Join a session\n"));
        AJ_AlwaysPrintf(("leave <sessionId>                                    "
                         "         - Leave a session\n"));
        AJ_AlwaysPrintf(("chat <sessionId> <msg>                               "
                         "         - Send a message over a given session\n"));
        AJ_AlwaysPrintf(("schat <msg>                                          "
                         "         - Send a sessionless message\n"));
        AJ_AlwaysPrintf(("cancelsessionless <serialNum>                        "
                         "         - Cancel a sessionless message\n"));
        AJ_AlwaysPrintf(("addmatch <rule>                                      "
                         "         - Add a DBUS rule\n"));
        AJ_AlwaysPrintf(("removematch <rule>                                   "
                         "         - Remove a DBUS rule\n"));
        AJ_AlwaysPrintf(
            ("sendttl <ttl>                                                 - "
             "Set default ttl (in ms) for all chat messages (0 = infinite)\n"));
        AJ_AlwaysPrintf(("exit                                                 "
                         "         - Exit this program\n"));
        AJ_AlwaysPrintf(("\n"));
        continue;
      } else {
    ...
    if (status == AJ_OK) {
      ...
      case APP_CHAT_SIGNAL:
        status = AppHandleChatSignal_sessions(&msg);
        break;
      ...
      case AJ_SIGNAL_FOUND_ADV_NAME: {
        ...
        AJ_AlwaysPrintf(
            ("FoundAdvertisedName name=%s, namePrefix=%s\n", name, namePrefix));
      } break;
    ...
    if ((status == AJ_ERR_SESSION_LOST) || (status == AJ_ERR_READ) ||
      ...
      AJ_AlwaysPrintf(
          ("Disconnected from Daemon:%s\n", AJ_GetUniqueName(&bus)));
      AJ_Disconnect(&bus);
      connected_sessions = FALSE;
      /*
...
#ifdef AJ_MAIN
int sessions1() { return AJ_Main_sessions(); }
#endif
