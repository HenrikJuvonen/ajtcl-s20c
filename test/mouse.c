...
static const char *testInterface[] = {"org.alljoyn.ajlite_test",
                                      "!ADC_Update >i", "!Gyro_Update >i >i",
                                      "!Button_Down >i", NULL};
...
static const AJ_Object AppObjects[] = {
    {"/org/alljoyn/ajlite_test", testInterfaces}, {NULL}};
...
static void AppDoWork(AJ_BusAttachment *bus) {
  ...
  if (initialized == FALSE) {
    ...
    /*Disable self test*/
    GPIO_PinModeSet(ACCEL_SELFTEST_PORT, ACCEL_SELFTEST_PIN, gpioModePushPull,
                    0);
  }
...
int AJ_Main(void) {
  ...
  while (TRUE) {
    ...
    if (!connected) {
      ...
      if (status != AJ_OK) {
        AJ_AlwaysPrintf(
            ("AllJoyn failed to connect sleeping for 15 seconds\n"));
        AJ_Sleep(15 * 1000);
        continue;
      }
