...
/// globals
uint8_t connected_sessionslite = FALSE;
 ...
 */
static const char *commands[] = {"wait 1000",
...
void AuthCallback_sessionslite(const void *context, AJ_Status status) {
  *((AJ_Status *)context) = status;
}

AJ_Status AppSendChatSignal(AJ_BusAttachment *bus, uint32_t sessionId,
  ...
  while (TRUE) {
    ...
    if (commands[i] != NULL) {
      ...
      if (0 == strcmp("startservice", command)) {
        ...
        status = AppSendChatSignal(&bus, 0, chatMsg, ALLJOYN_FLAG_SESSIONLESS,
                                   sendTTL);
...
#ifdef AJ_MAIN
int sessionslite() { return AJ_Main_sessionslite(); }
#endif
